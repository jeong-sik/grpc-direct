...
module Protoc = struct
  ...
  let generate_file_content services =
    services |> List.map Grpc_protoc.Codegen.generate_service |> String.concat "\n"
  ;;

  let encode_file ~name ~content =
    let fields = [ 1, Field.length_delimited name; 15, Field.length_delimited content ] in
  ...
  let generate (req : request) : (string * string) list =
    let index =
      List.fold_left
        (fun acc (file : Google.Protobuf.FileDescriptorProto.t) ->
    ...
    in
    let find_file name = List.assoc_opt name index in
    file_targets
