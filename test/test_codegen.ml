...
let test_to_snake_case () =
  ...
  assert (to_snake_case "GetHTTPRequest" = "get_h_t_t_p_request");
  (* edge case *)
...
let test_generate_server () =
  ...
  in
  let code = Grpc_protoc.Codegen.generate_service service in
  assert (String.contains code 'S');
  (* Contains "Server" *)
  assert (String.contains code 'm');
  (* Contains "module type S" *)
