...
let test_tls_config_record () =
  ...
  let config = Grpc_eio.Tls_config.create
...
let test_mtls_config () =
  ...
  let config = Grpc_eio.Tls_config.create_mtls
    ~cert_file:"/path/to/cert.pem"
    ~key_file:"/path/to/key.pem"
...
let create_test_certs () =
  ...
  let cmd = Printf.sprintf
    "openssl req -x509 -newkey rsa:2048 -keyout %s -out %s -days 1 -nodes -subj '/CN=localhost' 2>/dev/null"
    key_file cert_file
  in
  if Sys.command cmd = 0 then Some (cert_file, key_file)
  else None

let cleanup_test_certs cert_file key_file =
  (try Sys.remove cert_file with _ -> ());
  (try Sys.remove key_file with _ -> ())

let test_tls_config_with_real_certs () =
  match create_test_certs () with
  | None ->
      Printf.printf "  ⚠ Skipped (openssl not available)\n%!"
...
let test_client_tls_config () =
  ...
  let mtls = Grpc_eio.Client.tls_mtls
    ~ca_file:"/path/to/ca.pem"
...
let test_channel_credentials () =
  ...
  let creds = Grpc_eio.Client.Credentials.mtls
    ~ca_file:"/path/to/ca.pem"
    ~cert_file:"/path/to/client.pem"
    ~key_file:"/path/to/client.key"
    ()
  ...
  let creds = Grpc_eio.Client.Credentials.with_token
    ~token:"my-jwt-token"
    ()
  in
  assert (Grpc_eio.Client.Credentials.requires_tls creds);
  ...
  let creds = Grpc_eio.Client.Credentials.with_headers
...
let test_config_with_credentials () =
  ...
  let creds = Grpc_eio.Client.Credentials.mtls
    ...
    ()
  in
  let config = Grpc_eio.Client.config_with_credentials
