...
let test_constants () =
  assert (Flow_control.max_window_size = 2147483647);
  (* 2^31 - 1 *)
...
let test_update_overflow () =
  ...
  (try
     ...
     assert false
   with
   | Failure msg -> assert (String.length msg > 0));
...
let test_consume_recv_triggers_update () =
  ...
  Printf.printf
    "PASS consume_recv triggers update (got %d updates)\n%!"
    (List.length updates)
;;
...
let test_update_recv_initial_window_size () =
  ...
  @@ fun _env ->
  let fc = Flow_control.create () in
  let _ = Flow_control.stream_recv_window fc 1l in
  (* Create stream *)
