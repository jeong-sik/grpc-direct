...
let test_zstd () =
  ...
  let data =
    "Hello, gRPC! This is a test message for zstd compression. Compact Protocol v4!"
  in
  Printf.printf "📦 Original size: %d bytes\n%!" (String.length data);
  match codec.encoder data with
  | Ok encoded ->
...
let test_zstd_large () =
  ...
  | Ok encoded ->
    let ratio =
      float_of_int (String.length encoded) /. float_of_int (String.length data)
    in
    Printf.printf
      "📦 Zstd compressed: %d bytes (%.1f%%)\n%!"
      (String.length encoded)
      (ratio *. 100.);
    (match codec.decoder encoded with
     | Ok decoded ->
...
let test_message_framing_zstd () =
  let codec = Grpc_core.Codec.zstd () in
  let data =
    "Test message with zstd compression for gRPC framing - Compact Protocol v4"
  in
...
let test_message_framing () =
  ...
  | Ok frame ->
    Printf.printf
      "📦 Frame size: %d bytes (data: %d + header: 5)\n%!"
      (String.length frame)
      (String.length data);
    (match Grpc_core.Message.decode ~codec frame with
     | Ok decoded ->
