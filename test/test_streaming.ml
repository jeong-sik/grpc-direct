...
let test_streaming_accumulation () =
  ...
  let chunk_sizes = [ 3; 5; 4; 6; 4 ] in
  (* Split 22 bytes into chunks *)
  let buffer = ref "" in
  ...
  List.iter
    (fun size ->
       let chunk =
         String.sub full_data !pos (min size (String.length full_data - !pos))
       in
       buffer := !buffer ^ chunk;
       pos := !pos + size;
...
let test_extract_oversized () =
  ...
  | Ok _ -> failwith "Expected Oversized error"
