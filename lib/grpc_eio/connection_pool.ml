...
module TargetPool = struct
  ...
  let with_lock pool f =
    Eio.Mutex.use_rw ~protect:true pool.mutex f
  ...
  let find_idle pool =
    List.find_opt (fun c -> c.state = Idle) pool.connections
  ...
  let report_error pool conn =
    with_lock pool (fun () ->
      ...
      if conn.error_count > 3 then begin
        conn.state <- Closed;
        pool.connections <- List.filter (fun c -> c.id <> conn.id) pool.connections
      end
    )
  ...
  let cleanup_idle pool now =
    with_lock pool (fun () ->
      pool.connections <- List.filter (fun conn ->
        if conn.state = Idle && now -. conn.last_used > pool.config.max_idle_time then begin
          conn.state <- Closed;
          false
        end else
          conn.state <> Closed
...
let all_stats t =
  Eio.Mutex.use_ro t.mutex (fun () ->
    Hashtbl.fold (fun target pool acc ->
      (target, TargetPool.stats pool) :: acc
    ) t.pools []
  )
...
let print_stats t =
  ...
  Printf.printf "%-40s %6s %6s %6s %10s %8s\n"
    "Target" "Idle" "Busy" "Total" "Acquired" "Errors";
  Printf.printf "%s\n" (String.make 80 '-');
  List.iter (fun (target, (idle, busy, total, acquired, errors)) ->
