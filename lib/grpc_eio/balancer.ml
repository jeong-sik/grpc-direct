...
let create
  ...
  if List.length targets = 0 then
    failwith "At least one target is required";

  let weights = match strategy with
    | WeightedRoundRobin ws -> ws
    | _ -> List.map (fun _ -> 1) targets
  in

  let backends = List.map2 (fun target weight ->
    create_backend ~weight target
  ) targets weights |> Array.of_list in
...
let round_robin (balancer : t) : backend option =
  ...
  let rec find_next i count =
    if count >= n then None
      ...
      if is_available backend then begin
        balancer.current_index <- (i + 1) mod n;
        Some backend
      end else
        find_next ((i + 1) mod n) (count + 1)
  in
  find_next start 0
...
let weighted_round_robin (balancer : t) : backend option =
  ...
  let available = Array.to_list balancer.backends
    |> List.filter is_available in
  if List.length available = 0 then None
  else begin
    let total_weight = List.fold_left (fun acc b -> acc + b.weight) 0 available in
    if total_weight = 0 then None
    else begin
      ...
      let rec find_weighted backends acc =
        ...
        | b :: rest ->
            ...
            if idx < new_acc then Some b
            else find_weighted rest new_acc
      in
...
let random_select (balancer : t) : backend option =
  ...
  if n = 0 then None
  else Some available.(Random.int n)
...
let record_success (balancer : t) (backend : backend) : unit =
  Eio.Mutex.use_rw ~protect:true balancer.mutex (fun () ->
    ...
    if backend.state = Unhealthy then begin
      (* Recovery logic could be added here *)
      backend.state <- Healthy
    end else if backend.state = Unknown then
      backend.state <- Healthy
  )
...
let record_failure (balancer : t) (backend : backend) : unit =
  Eio.Mutex.use_rw ~protect:true balancer.mutex (fun () ->
    ...
    if backend.consecutive_failures >= balancer.config.failure_threshold then
      backend.state <- Unhealthy
  )
...
let mark_unhealthy (balancer : t) (target : string) : unit =
  Eio.Mutex.use_rw ~protect:true balancer.mutex (fun () ->
    Array.iter (fun b ->
      if String.equal b.target target then
        b.state <- Unhealthy
    ) balancer.backends
  )
...
let mark_healthy (balancer : t) (target : string) : unit =
  Eio.Mutex.use_rw ~protect:true balancer.mutex (fun () ->
    Array.iter (fun b ->
      if String.equal b.target target then begin
        b.state <- Healthy;
...
let to_string (balancer : t) : string =
  let strategy_str = match balancer.config.strategy with
    | PickFirst -> "PickFirst"
    | RoundRobin -> "RoundRobin"
    | WeightedRoundRobin _ -> "WeightedRoundRobin"
    | Random -> "Random"
  in
  let backends_str = Array.to_list balancer.backends
    |> List.map (fun b ->
         let state_str = match b.state with
           | Healthy -> "H"
           | Unhealthy -> "U"
           | Unknown -> "?"
         in
    ...
    |> String.concat ", "
  in
  Printf.sprintf "Balancer[%s, %d/%d healthy]: %s"
    strategy_str
