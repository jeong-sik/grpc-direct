...
let create_connection ~sw ~env (pool : t) : pooled_conn =
  let client_config =
    match pool.config.client_config with
    | Some c -> Some c
...
let size (pool : t) : int = Eio.Mutex.use_ro pool.mutex (fun () -> pool.total_count)

(** Get number of available (idle) connections *)
let available (pool : t) : int =
  Eio.Mutex.use_ro pool.mutex (fun () -> pool.total_count - pool.in_use_count)
;;
...
let stats_to_string (s : stats) : string =
  Printf.sprintf
    "Pool[total=%d, in_use=%d, idle=%d, acquired=%d, created=%d, evicted=%d]"
    s.total_connections
