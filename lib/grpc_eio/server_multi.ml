...
let serve_multi ?(domains = 0) ~env (server : Server.t) : unit =
  let num_domains =
    if domains > 0 then domains else max 1 (Domain.recommended_domain_count () - 1)
  in
  Log.info "🚀 Starting multi-domain gRPC server with %d domains" num_domains;
  (* Reset shutdown flag *)
  ...
  let domain_worker domain_id () =
    ...
    List.iter
      (fun service -> ignore (Server.add_service service local_server))
      services_list;
    let local_server =
      List.fold_left (fun s i -> Server.with_interceptor i s) local_server interceptors
    in
    Server.set_running local_server true;
    Log.info "  Domain %d: Listening on port %d" domain_id config.port;
    (* Each domain binds to the same port with SO_REUSEPORT *)
    let net = Eio.Stdenv.net env in
    let clock = Eio.Stdenv.clock env in
    let addr = `Tcp (Eio.Net.Ipaddr.V4.loopback, config.port) in
    let socket =
      Eio.Net.listen net ~sw ~backlog:128 ~reuse_addr:true ~reuse_port:true addr
    in
    let h2_handler =
    ...
    while Server.running local_server && not (Atomic.get global_shutdown) do
      Eio.Net.accept_fork
        ...
        ~sw
        ~on_error:(fun exn ->
          Log.error "Domain %d error: %s" domain_id (Printexc.to_string exn))
        handle_connection
    done;
    Log.info "  Domain %d: Stopped" domain_id
  in
  (* Spawn worker domains (1 to N-1) *)
  let worker_domains =
    List.init (num_domains - 1) (fun i ->
      Log.info "  Spawning domain %d..." (i + 1);
  ...
  (Server.set_running server true;
   Log.info "  Domain 0: Listening on port %d (main)" config.port;
   let net = Eio.Stdenv.net env in
   let clock = Eio.Stdenv.clock env in
   let addr = `Tcp (Eio.Net.Ipaddr.V4.loopback, config.port) in
   let socket =
   ...
   while Server.running server && not (Atomic.get global_shutdown) do
     Eio.Net.accept_fork
       socket
       ~sw
       ~on_error:(fun exn -> Log.error "Domain 0 error: %s" (Printexc.to_string exn))
       handle_connection
   done;
   Log.info "  Domain 0: Stopped (main)");
  (* Wait for all worker domains to finish *)
  List.iter Domain.join worker_domains;
  Log.info "All domains stopped"
;;
...
let shutdown (server : Server.t) : unit =
  Log.info "Shutting down multi-domain gRPC server...";
