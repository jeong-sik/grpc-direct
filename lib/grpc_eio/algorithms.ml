...
module P2C = struct
  ...
  let create backends =
    let backends = Array.map (fun (value, weight) -> {
      value;
      load = 0;
      total_requests = 0;
      failures = 0;
  ...
  let pick t =
    ...
    if n = 0 then None
    else if n = 1 then Some t.backends.(0)
    else begin
  ...
  let release t value =
    Array.iter (fun b ->
      if b.value == value && b.load > 0 then
        b.load <- b.load - 1
    ) t.backends

  let report_failure t value =
    Array.iter (fun b ->
      if b.value == value then begin
  ...
  let stats t =
    Array.map (fun b ->
      (b.value, b.load, b.total_requests, b.failures)
    ) t.backends
end
...
module RingBuffer = struct
  ...
  let try_push t item =
    ...
    if producer_pos - consumer_pos >= t.capacity then
      false  (* Buffer full *)
    else begin
  ...
  let try_pop t =
    let consumer_pos = Atomic.get t.consumer_cursor in
    let producer_pos = Atomic.get t.producer_cursor in
    if consumer_pos >= producer_pos then
      None  (* Buffer empty *)
    else begin
  ...
  let is_empty t =
    Atomic.get t.consumer_cursor >= Atomic.get t.producer_cursor

  let is_full t =
    Atomic.get t.producer_cursor - Atomic.get t.consumer_cursor >= t.capacity

  let size t =
    Atomic.get t.producer_cursor - Atomic.get t.consumer_cursor
end
...
module AdaptiveBatching = struct
  ...
  let add t item : 'a list option =
    ...
    let should_flush =
      t.pending_count >= t.current_batch_size ||
      elapsed_ms >= t.config.max_delay_ms
    in
  ...
  let report_latency t latency_ms =
    ...
    if t.avg_latency > t.config.target_latency_ms *. 1.5 then
      (* Latency too high, reduce batch size *)
      t.current_batch_size <- max t.config.min_batch_size (t.current_batch_size - 1)
    else if t.avg_latency < t.config.target_latency_ms *. 0.5 then
  ...
  let stats t =
    (t.current_batch_size, t.avg_latency, t.total_batches)
end
...
module ExpHistogram = struct
  ...
  let value_to_bucket t value =
    if value < t.min_value then 0
    else if value > t.max_value then Array.length t.buckets - 1
    else
      let decades = log10 (value /. t.min_value) in
      min (Array.length t.buckets - 1) (int_of_float (decades *. float buckets_per_decade))
  ...
  let percentile t p =
    ...
    let rec find_bucket acc i =
      if i >= Array.length t.buckets then
        bucket_to_value t (Array.length t.buckets - 1)
      else
        let acc = acc + t.buckets.(i) in
        if acc >= target then bucket_to_value t i
        else find_bucket acc (i + 1)
    in
    find_bucket 0 0

  let mean t =
    if t.count = 0 then 0.0 else t.sum /. float t.count
...
module CircuitBreaker = struct
  ...
  let allow t =
    ...
    | Closed ->
        ...
        true
    | Open ->
        if now -. t.last_failure_time >= t.config.timeout_ms then begin
    ...
    | HalfOpen ->
        if t.half_open_count < t.config.half_open_max then begin
  ...
  let record_success t =
    ...
    | HalfOpen ->
        ...
        if t.success_count >= t.config.success_threshold then begin
          t.state <- Closed;
          t.failure_count <- 0;
          t.success_count <- 0;
          t.half_open_count <- 0
  ...
  let record_failure t =
    ...
    | Closed ->
        ...
        if t.failure_count >= t.config.failure_threshold then begin
          t.state <- Open;
          t.failure_count <- 0
        end
    | HalfOpen ->
  ...
  let stats t =
    (state_name t.state, t.total_allowed, t.total_rejected, t.failure_count)
end
