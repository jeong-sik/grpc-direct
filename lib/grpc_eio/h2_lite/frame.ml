...
let parse_header (buf : Cstruct.t) : header * Cstruct.t =
  if Cstruct.length buf < header_size then invalid_arg "Buffer too small for frame header";
...
let parse (buf : Cstruct.t) : (t * Cstruct.t) option =
  ...
  then None
    ...
    if Cstruct.length rest < header.length
    then None
    else (
...
let make_data ~stream_id ~end_stream (payload : Cstruct.t) : t =
  ...
  { header = { length = Cstruct.length payload; frame_type = Data; flags; stream_id }
  ; payload
  }
;;
...
let make_headers ~stream_id ~end_stream ~end_headers (payload : Cstruct.t) : t =
  let flags =
    (if end_stream then Flags.end_stream else 0)
    lor if end_headers then Flags.end_headers else 0
  in
  { header = { length = Cstruct.length payload; frame_type = Headers; flags; stream_id }
  ; payload
  }
;;

(** Create HEADERS frame with priority info (RFC 7540 §5.3.1) *)
let make_headers_with_priority
      ~stream_id
      ~end_stream
      ~end_headers
  ...
  then invalid_arg "Priority weight out of range (1-256)";
  ...
  let flags =
    Flags.priority
    lor (if end_stream then Flags.end_stream else 0)
    lor if end_headers then Flags.end_headers else 0
  in
  { header = { length = total_len; frame_type = Headers; flags; stream_id }
  ; payload = combined
  }
;;
...
let make_priority ~stream_id (priority : priority_info) : t =
  ...
  { header = { length = 5; frame_type = Priority; flags = 0; stream_id }; payload }
;;

(** Parse priority fields from payload (RFC 7540 §6.3) *)
let parse_priority (payload : Cstruct.t) ~offset : priority_info * int =
  if Cstruct.length payload < offset + 5 then invalid_arg "PRIORITY payload too small";
...
let make_push_promise ~stream_id ~promised_stream_id ~end_headers (payload : Cstruct.t)
  : t
  =
...
let make_rst_stream ~stream_id ~error_code : t =
  ...
  { header = { length = 4; frame_type = RstStream; flags = 0; stream_id }; payload }
;;
...
let make_window_update ~stream_id ~increment : t =
  ...
  { header = { length = 4; frame_type = WindowUpdate; flags = 0; stream_id }; payload }
;;
...
let make_goaway ~last_stream_id ~error_code ~debug_data : t =
  ...
  { header = { length = 8 + debug_len; frame_type = GoAway; flags = 0; stream_id = 0l }
  ; payload
  }
;;
...
let make_continuation ~stream_id ~end_headers (payload : Cstruct.t) : t =
  ...
  { header =
      { length = Cstruct.length payload; frame_type = Continuation; flags; stream_id }
  ; payload
  }
;;
...
let make_headers_fragmented
  ...
  if first_max <= 0 then invalid_arg "max_frame_size too small for priority block";
  if total_len <= first_max
  then
...
let make_push_promise_fragmented
      ~stream_id
      ~promised_stream_id
      ?(max_frame_size = 16384)
      (header_block : Cstruct.t)
  ...
  if first_max <= 0 then invalid_arg "max_frame_size too small for PUSH_PROMISE";
  if total_len <= first_max
  then [ make_push_promise ~stream_id ~promised_stream_id ~end_headers:true header_block ]
  else (
    let frames = ref [] in
    let offset = ref 0 in
    let first = ref true in
    while !offset < total_len do
      ...
      let frame =
        ...
        else make_continuation ~stream_id ~end_headers:is_last chunk
      in
      frames := frame :: !frames;
      offset := !offset + chunk_size
    done;
