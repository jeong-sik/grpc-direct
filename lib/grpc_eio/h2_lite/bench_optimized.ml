...
let bench_huffman () =
  ...
  Printf.printf
    "Encode: %.2f ms total, %.4f µs/op\n"
    encode_time
    (encode_time *. 1000. /. float total_ops);
  Printf.printf
    "Decode: %.2f ms total, %.4f µs/op\n"
    decode_time
    (decode_time *. 1000. /. float total_ops);
  (* Show compression *)
  Printf.printf "\nCompression ratios:\n";
  List.iter2
    (fun orig enc ->
...
let bench_buffer_pool () =
  ...
  Printf.printf
    "Pooled:   %.2f ms total, %.4f µs/op\n"
    pool_time
    (pool_time *. 1000. /. float ops);
  Printf.printf
    "Standard: %.2f ms total, %.4f µs/op\n"
    alloc_time
    (alloc_time *. 1000. /. float ops);
...
let bench_hpack_static () =
  ...
  let lookup_time =
    time_ms (fun () ->
      for _ = 1 to iterations * 10 do
        List.iter (fun (name, _value) -> ignore (Hpack.lookup_static_name name)) headers
      done)
  in
  let ops = iterations * 10 * List.length headers in
  Printf.printf "Iterations: %d lookups\n" ops;
  Printf.printf
    "Lookup time: %.2f ms total, %.4f µs/op\n"
    lookup_time
    (lookup_time *. 1000. /. float ops)
;;
...
let bench_frame_parsing () =
  ...
  Printf.printf "Iterations: %d\n" ops;
  Printf.printf
    "Parse time: %.2f ms total, %.4f µs/op\n"
    parse_time
    (parse_time *. 1000. /. float ops);
...
let bench_hpack_encode () =
  ...
  Printf.printf "Iterations: %d\n" iterations;
  Printf.printf
    "Encode time: %.2f ms total, %.4f µs/op\n"
    encode_time
    (encode_time *. 1000. /. float iterations);
