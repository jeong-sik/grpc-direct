...
module Multiplexer = struct
  ...
  let create () = { streams = Hashtbl.create 64; next_server_stream_id = 2l }
    ...
    while not conn.closed do
      ...
      | H2_lite.Frame.Data ->
        ...
        then ()
          ...
          let data_frame =
            H2_lite.Frame.make_data ~stream_id ~end_stream:false encoded_body
          in
          ...
          let trailers_frame =
            H2_lite.Frame.make_headers
              ...
              encoded_trailers
          in
          H2_lite.Connection.write_frames
            conn
            [ headers_frame; data_frame; trailers_frame ];
      ...
      | H2_lite.Frame.Settings -> H2_lite.Connection.send_settings_ack conn
      | H2_lite.Frame.Ping
        when H2_lite.Frame.Flags.is_set
               frame.H2_lite.Frame.header.flags
               H2_lite.Frame.Flags.ack -> ()
      | H2_lite.Frame.Ping -> H2_lite.Connection.send_ping conn ~ack:true frame.payload
      | H2_lite.Frame.WindowUpdate -> ()
      | H2_lite.Frame.GoAway -> conn.goaway_received <- true
      | H2_lite.Frame.RstStream -> Multiplexer.remove mux stream_id
      | _ -> ()
    done
  with
...
let domain_worker ~port ~domain_id () =
  ...
  let socket =
    Eio.Net.listen net ~sw ~backlog:1024 ~reuse_addr:true ~reuse_port:true addr
  in
  ...
  while not (Atomic.get global_shutdown) do
    Eio.Net.accept_fork socket ~sw ~on_error:(fun _exn -> ()) handle_connection
  done;
