...
let bench_buffer_pool () =
  ...
  Printf.printf
    "Pooled:   %.2f µs total, %.4f µs/op\n"
    pooled_time
    (pooled_time /. float iterations);
  Printf.printf
    "Standard: %.2f µs total, %.4f µs/op\n"
    standard_time
    (standard_time /. float iterations);
...
let bench_frame_parsing () =
  ...
  Printf.printf "Iterations: %d\n" iterations;
  Printf.printf
    "Parse time: %.2f µs total, %.4f µs/op\n"
    parse_time
    (parse_time /. float iterations);
...
let bench_frame_serialization () =
  ...
  Printf.printf "Iterations: %d\n" iterations;
  Printf.printf
    "Serialize time: %.2f µs total, %.4f µs/op\n"
    serialize_time
    (serialize_time /. float iterations);
...
let bench_grpc_message () =
  ...
  Printf.printf "Iterations: %d\n" iterations;
  Printf.printf
    "Encode time: %.2f µs total, %.4f µs/op\n"
    encode_time
    (encode_time /. float iterations);
...
let bench_request_cycle () =
  ...
  let cycle_time =
    time_us (fun () ->
      for i = 1 to iterations do
        ...
        (* Encode headers *)
        let headers_len =
          Hpack_lite.encode_request_headers
            ~scheme:"http"
        ...
        let _hf =
          Frame.make_headers ~stream_id ~end_stream:false ~end_headers:true headers
        in
        (* Encode body *)
  ...
  Printf.printf "Iterations: %d\n" iterations;
  Printf.printf
    "Cycle time: %.2f µs total, %.4f µs/op\n"
    cycle_time
    (cycle_time /. float iterations);
  Printf.printf
    "Throughput: %.2f M req/sec (encoding only)\n"
