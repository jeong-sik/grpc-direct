...
let test_grpc_headers () =
  ...
  List.iter (fun h ->
    if not (test_roundtrip h) then all_pass := false
  ) headers;
  if !all_pass then
    Printf.printf "✓ gRPC headers: roundtrip OK\n"
  else
    Printf.printf "✗ gRPC headers: some failed\n";
  !all_pass
...
let test_ascii () =
  ...
  if !all_pass then
    Printf.printf "✓ ASCII (32-126): roundtrip OK\n"
  else
    Printf.printf "✗ ASCII: some failed\n";
  !all_pass
...
let test_compression () =
  ...
  List.iter (fun (s, desc) ->
    ...
    let ratio = float_of_int (String.length encoded) /. float_of_int (String.length s) *. 100.0 in
    Printf.printf "  %s: %d -> %d bytes (%.1f%%)\n" desc (String.length s) (String.length encoded) ratio
  ) test_cases;
...
let test_table_sanity () =
  ...
  let expected = [
    (* Common ASCII *)
    (0x30, (0x0, 5));   (* '0' = 0x30 = 48 *)
    (0x61, (0x3, 5));   (* 'a' = 0x61 = 97 *)
    (0x65, (0x5, 5));   (* 'e' = 0x65 = 101 *)
    (0x20, (0x14, 6));  (* space = 0x20 = 32 *)
    (* High bytes that were broken *)
    (0xC0, (0x3ffffe0, 26));  (* 192 *)
    (0xD0, (0x7fff2, 19));    (* 208 *)
    (0xE0, (0xfffec, 20));    (* 224 *)
    (0xF0, (0x3ffffeb, 26));  (* 240 *)
    (0xFF, (0x3ffffee, 26));  (* 255 *)
  ] in
  let all_pass = ref true in
  List.iter (fun (sym, (expected_code, expected_len)) ->
    let (actual_code, actual_len) = Hpack.Huffman.table.(sym) in
    if actual_code <> expected_code || actual_len <> expected_len then begin
      Printf.printf "FAIL: table[%d] = (0x%x, %d), expected (0x%x, %d)\n"
        sym actual_code actual_len expected_code expected_len;
      all_pass := false
    end
  ) expected;
  if !all_pass then
    Printf.printf "✓ Huffman table: spot check OK\n"
  else
    Printf.printf "✗ Huffman table: errors found\n";
  !all_pass
...
let test_encode_string () =
  ...
  let (decoded, _) = Hpack.decode_string buf ~offset:0 in
  if decoded <> sample then begin
    Printf.printf "✗ encode_string roundtrip failed: %S -> %S\n" sample decoded;
    false
  end else if used_huffman <> expect_huffman then begin
    Printf.printf "✗ encode_string Huffman flag mismatch (expected %b, got %b)\n"
      expect_huffman used_huffman;
    false
  end else begin
...
let () =
  ...
  if passed = total then
    Printf.printf "✅ All %d Huffman tests passed!\n" total
  else
