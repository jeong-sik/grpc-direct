...
let consume t stream_id size =
  ...
  Eio.Mutex.use_rw sw.mutex ~protect:false (fun () -> sw.size <- sw.size - size)
;;
...
let update t stream_id increment =
  ...
  then
    ...
    Eio.Mutex.use_rw t.send_window.mutex ~protect:false (fun () ->
      ...
      if new_size > max_window_size then failwith "FLOW_CONTROL_ERROR: window overflow";
...
let consume_recv t stream_id size =
  let threshold = t.recv_window.initial / 2 in
  (* Update at 50% *)
  ...
  let updates = ref [] in
  if t.recv_window.size < threshold
  then (
  ...
  Hashtbl.iter
    (fun _ sw ->
       Eio.Mutex.use_rw sw.mutex ~protect:false (fun () ->
         ...
         sw.initial <- new_size;
         if sw.size > 0 then Eio.Condition.broadcast sw.condition))
