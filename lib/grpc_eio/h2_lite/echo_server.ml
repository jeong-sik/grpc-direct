  ...
  | Some ("1" | "true" | "on") ->
    ...
    := Some
         (Gc.create_alarm (fun () ->
            ...
            if st.Gc.major_collections > !last_major
            then (
              last_major := st.Gc.major_collections;
              let elapsed = Unix.gettimeofday () -. start_time in
              traceln
                "GC major=%d minor=%d heap_words=%d live_words=%d time=%.3fs"
...
module Multiplexer = struct
  ...
  let create () =
    { streams = Hashtbl.create 64; next_server_stream_id = 2l (* Server uses even IDs *) }
  ;;
  ...
  let get_or_create t stream_id =
    ...
    | None ->
      let entry =
        { state = H2_lite.Stream.Open; pending_data = []; window_size = 65535 }
      in
      Hashtbl.add t.streams stream_id entry;
      entry
  ;;

  let remove t stream_id = Hashtbl.remove t.streams stream_id
  let _active_count t = Hashtbl.length t.streams
end
...
let handle_connection_full flow _addr =
  ...
  try
    ...
    let local_header_table_size =
      conn.H2_lite.Connection.local_settings.header_table_size
    in
    ...
    while not conn.closed do
      ...
      | H2_lite.Frame.Data ->
        ...
        if data_len < 5
        then () (* gRPC header is 5 bytes minimum *)
        else (
          ...
          (match outlier_ms with
           | Some threshold ->
             ...
             then
               traceln
                 "WriteOutlier: stream=%ld len=%d ms=%.3f"
                 stream_id
                 (Cstruct.length request_body)
                 elapsed_ms
           | None -> H2_lite.Connection.write_echo_response conn ~stream_id request_body);
          (* Mark stream as done *)
        ...
        (match handle_t0, outlier_ms with
         | Some t0, Some threshold ->
           ...
           then
             traceln
               "HandleOutlier: stream=%ld len=%d ms=%.3f"
               stream_id
               data_len
               elapsed_ms
         | _ -> ())
      | H2_lite.Frame.Settings
        when H2_lite.Frame.Flags.is_set
      ...
      | H2_lite.Frame.Settings ->
        ...
        List.iter
          (fun (id, value) ->
             if id = 1
             then (* SETTINGS_HEADER_TABLE_SIZE *)
        ...
        then (
          ...
          then (
            ...
            let write_response () =
              if window_updates
              then
                H2_lite.Connection.write_echo_response_with_window_update
                  conn
                  ~stream_id
                  request_body
                  ~window_increment
              else H2_lite.Connection.write_echo_response conn ~stream_id request_body
            in
            match outlier_ms with
            | Some threshold ->
              let t0 = Unix.gettimeofday () in
              ...
              then
                traceln
                  ...
                  elapsed_ms
            | None -> write_response ())
          else if window_updates && window_increment > 0
          then
            H2_lite.Connection.send_window_update
              conn
              ~stream_id:0l
...
let run_server ~env ~port =
  ...
  let use_fast =
    ...
    | _ -> true
  in
  let handler = if use_fast then handle_connection_fast else handle_connection_full in
  Switch.run
  @@ fun sw ->
  ...
  traceln
    "Benchmark with: ghz --insecure --proto echo.proto --call echo.EchoService/Echo -d \
     '{\"message\":\"hello\"}' -c 50 -n 10000 localhost:%d"
    port;
  (* Pre-warm buffer pools *)
  H2_lite.Buffer_pool.prewarm ~count_per_class:128;
  (* Accept loop *)
  while true do
    Eio.Net.accept_fork
      socket
