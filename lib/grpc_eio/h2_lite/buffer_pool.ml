...
let size_class_index size =
  ...
  if size <= 256 then 0
  else if size <= 512 then 1
  else if size <= 1024 then 2
  else if size <= 2048 then 3
  else if size <= 4096 then 4
  else if size <= 8192 then 5
  else if size <= 16384 then 6
  else 7
...
let release buf : unit =
  ...
  if size >= 256 && size <= 32768 then begin
    let idx = size_class_index size in
    let ring = pool.pools.(idx) in

    if size = ring.size && ring.count < pool_capacity then begin
      (* Add to ring buffer *)
