...
let set_priority t (priority : Frame.priority_info) =
  if priority.dependency = t.id then
    raise (Stream_error (ProtocolError "PRIORITY dependency on self"));
  if priority.weight < 1 || priority.weight > 256 then
    raise (Stream_error (ProtocolError "PRIORITY weight out of range"));
  t.priority <- priority
...
let transition_send t frame_type ~end_stream =
  ...
  (* From idle *)
  | Idle, `Headers ->
    t.state <- if end_stream then HalfClosedLocal else Open
  | Idle, `PushPromise ->
    t.state <- ReservedLocal

  (* From reserved (local) *)
  | ReservedLocal, `Headers ->
    t.state <- if end_stream then Closed else HalfClosedRemote
  | ReservedLocal, `RstStream ->
    t.state <- Closed

  (* From open *)
  | Open, `Data when end_stream ->
    t.state <- HalfClosedLocal
  | Open, `Headers when end_stream ->
    t.state <- HalfClosedLocal
  | Open, `RstStream ->
    t.state <- Closed
  | Open, _ -> ()  (* Stay open *)
  ...
  | HalfClosedRemote, `Data when end_stream ->
    t.state <- Closed
  | HalfClosedRemote, `Headers when end_stream ->
    t.state <- Closed
  | HalfClosedRemote, `RstStream ->
    t.state <- Closed
  | HalfClosedRemote, _ -> ()

  (* From half-closed (local) - we already sent END_STREAM *)
  | HalfClosedLocal, `RstStream ->
    t.state <- Closed
  ...
  | Closed, _ ->
    raise (Stream_error StreamClosed)

  | state, frame_type ->
    let frame_name = match frame_type with
      | `Headers -> "HEADERS"
      | `Data -> "DATA"
...
let transition_recv t frame_type ~end_stream =
  ...
  | HalfClosedLocal, `Data when end_stream ->
    t.state <- Closed
  | HalfClosedLocal, `Headers when end_stream ->
    t.state <- Closed
  | HalfClosedLocal, `RstStream ->
    t.state <- Closed
  | HalfClosedLocal, _ -> ()
...
let consume_send_window t bytes =
  if bytes > t.send_window then
    raise (Stream_error (ProtocolError "send window exhausted"));
...
let update_send_window t bytes =
  ...
  if t.send_window > 2147483647 then
    raise (Stream_error (ProtocolError "flow control window overflow"))

(** Append data frame payload *)
let append_data t payload =
  t.data <- t.data @ [payload]

(** Get accumulated data *)
let get_data t =
  let total_len = List.fold_left (fun acc cs -> acc + Cstruct.length cs) 0 t.data in
  let result = Cstruct.create total_len in
  let _ = List.fold_left (fun off cs ->
