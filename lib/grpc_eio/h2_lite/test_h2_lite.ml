...
let test_frame_make_headers () =
  ...
  let frame =
    Frame.make_headers ~stream_id:3l ~end_stream:false ~end_headers:true headers
  in
...
let test_frame_roundtrip () =
  ...
  | None -> failwith "Failed to parse serialized frame"
;;

let test_frame_ping () =
  let opaque = Cstruct.create 8 in
  for i = 0 to 7 do
    Cstruct.set_uint8 opaque i (i + 1)
  done;
