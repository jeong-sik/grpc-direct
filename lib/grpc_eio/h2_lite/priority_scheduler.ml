...
let create () : t = { nodes = Hashtbl.create 32; active = [] }
...
let update_priority (t : t) ~stream_id (priority : Frame.priority_info) =
  ...
  node.weight <- weight;
  if node.remaining > node.weight then node.remaining <- node.weight
;;
...
let enqueue (t : t) ~stream_id (frames : Frame.t list) =
  ...
  then (
    ...
    if was_empty then t.active <- t.active @ [ stream_id ])
;;

let has_pending (t : t) = t.active <> []
...
let pop_next (t : t) : Frame.t list option =
  let rec cleanup acc = function
    ...
    | id :: rest ->
      ...
      if Queue.is_empty node.queue then cleanup acc rest else cleanup (id :: acc) rest
  in
  t.active <- cleanup [] t.active;
  if t.active = []
  then None
  else (
    let all_zero = List.for_all (fun id -> (get_node t id).remaining <= 0) t.active in
    if all_zero
    then
      List.iter
        (fun id ->
    ...
    let rec pick prefix = function
      ...
      | id :: rest ->
        ...
        then (
          ...
          let new_active =
            if Queue.is_empty node.queue
            then List.rev_append prefix rest
            else List.rev_append prefix (rest @ [ id ])
          in
          t.active <- new_active;
          Some batch)
        else pick (id :: prefix) rest
    in
