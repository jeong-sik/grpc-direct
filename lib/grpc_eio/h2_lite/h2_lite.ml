...
let open_stream ~is_client ~hpack_encoder ~hpack_decoder ~flow_control (conn : Connection.t) : stream =
...
let send_data stream ~end_stream (data : Cstruct.t) =
  ...
  if not (Flow_control.can_send stream.flow_control stream.id data_len) then
    failwith "FLOW_CONTROL_ERROR: send window exhausted";
...
let send_headers ?priority stream ~end_stream (headers : (string * string) list) =
  ...
  if max_header_list_size >= 0 then begin
    let size = Hpack.header_list_size headers in
    if size > max_header_list_size then
      invalid_arg "Header list size exceeds peer SETTINGS_MAX_HEADER_LIST_SIZE"
  end;
  ...
  let frames =
    Frame.make_headers_fragmented ~stream_id:stream.id ~end_stream ?priority
      ~max_frame_size:(Connection.peer_max_frame_size stream.conn) encoded
  in
  match frames with
  | [] -> ()
...
let send_push_promise stream ~(headers : (string * string) list) : stream =
  if not stream.conn.Connection.local_settings.enable_push then
    invalid_arg "PUSH_PROMISE disabled by local settings";
  let promised_stream_id = Connection.next_stream_id stream.conn ~is_client:false in
  let encoded = Hpack.encode stream.hpack_encoder headers in
  let frames =
    Frame.make_push_promise_fragmented ~stream_id:stream.id
      ~promised_stream_id
...
let handle_connection_frame conn flow_control hpack_encoder (frame : Frame.t) =
  ...
  | Frame.Ping when Frame.Flags.is_set frame.Frame.header.flags Frame.Flags.ack ->
    ()  (* Ping ACK *)
  | Frame.Ping ->
    Connection.send_ping conn ~ack:true frame.Frame.payload
  | Frame.WindowUpdate ->
    if Cstruct.length frame.Frame.payload <> 4 then
      raise (Connection.Connection_error
    ...
    if increment = 0 then begin
      if frame.header.stream_id = 0l then
        raise (Connection.Connection_error
                 (Frame.Error_code.protocol_error, "WINDOW_UPDATE increment 0"))
      else begin
        let rst = Frame.make_rst_stream ~stream_id:frame.header.stream_id
          ~error_code:Frame.Error_code.flow_control_error in
        Connection.write_frame conn rst;
        Flow_control.remove_stream flow_control frame.header.stream_id
      end
    end else
      Flow_control.update flow_control frame.header.stream_id increment
  | Frame.GoAway ->
    conn.Connection.goaway_received <- true
  | _ ->
    ()
...
let rec recv_frame ?on_push stream : Frame.t =
  let decode_headers_for_stream stream frame =
    ...
    if max_header_list_size >= 0 then begin
      let size = Hpack.header_list_size headers in
      if size > max_header_list_size then begin
        let rst = Frame.make_rst_stream ~stream_id:stream.id
          ~error_code:Frame.Error_code.protocol_error in
  ...
  let apply_priority prio =
    ...
    with Stream.Stream_error (Stream.ProtocolError msg) ->
      let rst = Frame.make_rst_stream ~stream_id:stream.id
        ~error_code:Frame.Error_code.protocol_error in
      Connection.write_frame stream.conn rst;
      Flow_control.remove_stream stream.flow_control stream.id;
  ...
  let handle_window_update frame =
    if Cstruct.length frame.Frame.payload <> 4 then
      failwith "Invalid WINDOW_UPDATE payload";
    let raw = Cstruct.BE.get_uint32 frame.Frame.payload 0 in
    let increment = Int32.logand raw 0x7FFFFFFFl |> Int32.to_int in
    if increment = 0 then begin
      let rst = Frame.make_rst_stream ~stream_id:frame.header.stream_id
      ...
      Flow_control.update stream.flow_control frame.header.stream_id increment
  in
  let handle_push_promise info =
    ...
    in
    if not stream.conn.Connection.local_settings.enable_push then begin
      Connection.send_goaway stream.conn
    ...
    match on_push with
    | None ->
      let rst = Frame.make_rst_stream ~stream_id:promised_id
        ~error_code:Frame.Error_code.cancel in
      Connection.write_frame stream.conn rst;
    ...
    | Some handler ->
      ...
      let rec read_response () =
        ...
        (match frame.header.frame_type with
         ...
         | Frame.RstStream ->
           failwith "Received RST_STREAM on pushed stream"
         | _ -> ());
  ...
  let rec loop () =
    ...
    | Frame.WindowUpdate when frame.Frame.header.stream_id = stream.id ->
      ...
      loop ()
    | _ ->
      if frame.Frame.header.stream_id = stream.id then begin
        let frame =
          match frame.header.frame_type with
          | Frame.Headers ->
            let info = Connection.read_header_block ~first_frame:frame stream.conn in
            (match info.priority with
             ...
             | None -> ());
            let flags =
              (frame.header.flags lor Frame.Flags.end_headers)
              land (lnot (Frame.Flags.priority lor Frame.Flags.padded))
            in
            let header = {
              frame.header with
              length = Cstruct.length info.header_block;
              flags;
            } in
        ...
        let frame_type = match frame.header.frame_type with
          | Frame.Data -> `Data
        ...
        (match frame.header.frame_type with
         | Frame.Data ->
           let updates =
             Flow_control.consume_recv stream.flow_control stream.id (Cstruct.length frame.Frame.payload)
           in
           if updates <> [] then
         ...
         | _ -> ());
        frame
    end else if frame.Frame.header.stream_id = 0l then begin
      ...
      (match frame.header.frame_type with
       | Frame.Headers ->
           let info = Connection.read_header_block ~first_frame:frame stream.conn in
           (match info.priority with
            | Some prio -> Connection.update_priority stream.conn ~stream_id:info.stream_id prio
            | None -> ())
       | Frame.Priority ->
           let (prio, _) = Frame.parse_priority frame.Frame.payload ~offset:0 in
    ...
    if i >= len then
      ...
      | '%' when i + 2 < len ->
        ...
        (match int_of_string_opt ("0x" ^ hex) with
         | Some v -> Buffer.add_char buf (Char.chr v); loop (i + 3)
...
module Client = struct
  ...
  let connect ?(enable_push = false) ?(priority_scheduling = true) ?on_push
      ~sw ~env ~host ~port () : t =
    ...
    let settings =
      if enable_push then
        { Connection.grpc_settings with enable_push = true }
      else
        Connection.grpc_settings
    in
    let conn = Connection.create ~settings ~priority_scheduling flow in
    if priority_scheduling then
      Connection.start_priority_writer ~sw conn;
  ...
  let call_unary t ~service ~method_ ~request : Cstruct.t =
    let stream = open_stream ~is_client:true ~hpack_encoder:t.hpack_encoder ~hpack_decoder:t.hpack_decoder ~flow_control:t.flow_control t.conn in
    ...
    (match status with
     ...
     | None ->
       failwith "Missing grpc-status in response")

  (** Close client connection *)
  let close t =
    Connection.close t.conn
end
...
module Server = struct
  ...
  (** Start gRPC server *)
  let serve ?(enable_push = false) ?(priority_scheduling = true)
      ~sw ~env ~port (handler : handler) =
    let net = Eio.Stdenv.net env in
    let addr = `Tcp (Eio.Net.Ipaddr.V4.loopback, port) in
    ...
    let handle_connection flow _addr =
      ...
      if priority_scheduling then
        Connection.start_priority_writer ~sw conn;
      let hpack_decoder = Hpack.create () in  (* Per-connection HPACK *)
      let hpack_encoder = Hpack.create () in
      let flow_control = Flow_control.create () in
      try
          ...
          | Frame.Headers ->
            ...
            let headers_ok =
              if max_header_list_size >= 0 then begin
                ...
                true
            in
            if not headers_ok then begin
              let rst = Frame.make_rst_stream ~stream_id:info.stream_id
                ~error_code:Frame.Error_code.protocol_error in
              Connection.write_frame conn rst;
