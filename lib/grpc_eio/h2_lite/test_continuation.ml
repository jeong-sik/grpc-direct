...
let test_large_headers () =
  ...
  (* Small for testing *)
  let large_header = Cstruct.create 350 in
  (* Will need 4 frames *)
  (* Fill with test data *)
  for i = 0 to 349 do
  ...
  let frames =
    Frame.make_headers_fragmented ~stream_id ~end_stream:true ~max_frame_size large_header
  in
...
let test_data_integrity () =
  ...
  let frames =
    Frame.make_headers_fragmented ~stream_id ~end_stream:false ~max_frame_size original
  in
  (* Reassemble manually *)
  let total_len =
...
let test_empty_headers () =
  ...
  let frames = Frame.make_headers_fragmented ~stream_id ~end_stream:true empty_header in
  assert (List.length frames = 1);
  let frame = List.hd frames in
...
let test_frame_roundtrip () =
  ...
  let frames =
    Frame.make_headers_fragmented ~stream_id ~end_stream:true ~max_frame_size header_block
  in
