...
let parse_settings_payload payload =
  ...
  if len mod 6 <> 0 then
    failwith "Invalid SETTINGS payload length";
...
let apply_peer_settings t settings =
  List.iter (fun (id, value) ->
    ...
    | id when id = Frame.Settings_id.enable_push ->
      if value <> 0l && value <> 1l then
        failwith "Invalid SETTINGS_ENABLE_PUSH value";
    ...
    | id when id = Frame.Settings_id.initial_window_size ->
      if Int32.compare value 0x7FFFFFFFl > 0 then
        failwith "FLOW_CONTROL_ERROR: initial_window_size too large";
      let size = Int32.to_int value in
      t.peer_settings <- { t.peer_settings with initial_window_size = size }
    | id when id = Frame.Settings_id.max_frame_size ->
      let size = Int32.to_int value in
      if size < min_frame_size || size > max_frame_size then
        failwith "Invalid SETTINGS_MAX_FRAME_SIZE value";
    ...
    | _ ->
      ()  (* Ignore unknown settings per RFC 7540 *)
  ) settings
...
let recv_preface t =
  ...
  while t.read_pos < expected_len do
    let available = Cstruct.sub t.read_buf t.read_pos (Cstruct.length t.read_buf - t.read_pos) in
  ...
  if Cstruct.to_string preface_buf <> connection_preface then
    failwith "Invalid HTTP/2 connection preface";

  (* Shift remaining data to front of buffer *)
  let remaining_len = t.read_pos - expected_len in
  if remaining_len > 0 then begin
...
let fill_read_buffer t =
  ...
  if t.read_pos > 0 && Cstruct.length t.read_buf - t.read_pos < Frame.header_size then begin
...
let read_frame t : Frame.t =
  ...
  if header.length > t.local_settings.max_frame_size then
    raise (Connection_error (Frame.Error_code.frame_size_error, "frame too large"));
  ...
  | Some (frame, remaining) ->
    ...
    let last =
      if frame.Frame.header.stream_id = 0l then t.last_stream_id
      else if Int32.compare frame.Frame.header.stream_id t.last_stream_id > 0 then
...
let read_header_block ?first_frame t : header_block =
  let first_frame = match first_frame with
    | Some frame -> frame
    | None -> read_frame t
  in
  ...
  (match first_frame.header.frame_type with
   ...
   | Frame.PushPromise ->
       if end_stream then
         failwith "PUSH_PROMISE must not set END_STREAM"
  ...
  let header_fragment_of_headers_frame frame =
    ...
    let pad_len =
      if Frame.Flags.is_set frame.Frame.header.flags Frame.Flags.padded then begin
        if payload_len < 1 then
          failwith "Invalid HEADERS padding";
    ...
    let priority, promised_stream_id =
      ...
      | Frame.Headers ->
          if Frame.Flags.is_set frame.Frame.header.flags Frame.Flags.priority then begin
            let (prio, off2) = Frame.parse_priority payload ~offset:!off in
            if prio.dependency = stream_id then
              failwith "PRIORITY dependency on self";
            off := off2;
            (Some prio, None)
          end else
            (None, None)
      | Frame.PushPromise ->
          if payload_len < !off + 4 then
            failwith "Invalid PUSH_PROMISE payload";
          let promised = Cstruct.BE.get_uint32 payload !off |> Int32.logand 0x7FFFFFFFl in
          if promised = 0l then
            failwith "Invalid promised stream id 0";
          off := !off + 4;
          (None, Some promised)
      | _ -> (None, None)
    in
    ...
    if fragment_len < 0 then
      failwith "Invalid HEADERS padding length";
  ...
  if Frame.Flags.is_set first_frame.header.flags Frame.Flags.end_headers then
    ...
    let rec read_continuations () =
      ...
      if frame.header.stream_id <> stream_id then
        failwith "CONTINUATION frame on different stream";
      (* Verify frame type *)
      ...
      if not (Frame.Flags.is_set frame.header.flags Frame.Flags.end_headers) then
        read_continuations ()
    in
    read_continuations ();
    ...
    let _ = List.fold_left (fun offset cs ->
...
let write_frames_direct t (frames : Frame.t list) =
  ...
  let total_size = List.fold_left (fun acc frame ->
    acc + Frame.header_size + Cstruct.length frame.Frame.payload
  ) 0 frames in

  (* Fast path: if total fits in write_buf, copy everything and single write *)
  if total_size <= Cstruct.length t.write_buf then begin
    let offset = ref 0 in
    List.iter (fun (frame : Frame.t) ->
      (* Write header *)
    ...
    let iovecs = List.concat_map (fun (frame : Frame.t) ->
...
let is_control_frame (frame : Frame.t) =
  ...
  match frame.header.frame_type with
  | Frame.Settings | Frame.Ping | Frame.GoAway | Frame.WindowUpdate | Frame.RstStream -> true
  | _ -> false
...
let write_frames t (frames : Frame.t list) =
  ...
  | Some scheduler, Some signal ->
      ...
      let rec loop current_stream batch = function
        ...
        | frame :: rest ->
            if is_control_frame frame then begin
              flush_batch (List.rev batch);
              write_frame_direct t frame;
              loop None [] rest
            end else begin
...
let write_echo_response t ~stream_id (message : Cstruct.t) =
  ...
  let grpc_payload_len = 5 + msg_len in  (* gRPC header + message *)
  let total_len = 28 + 9 + grpc_payload_len + 24 in  (* HEADERS + DATA + TRAILERS *)

  if total_len > Cstruct.length t.write_buf then
    failwith "Message too large for write buffer";
  ...
  Cstruct.set_uint8 buf 2 19;  (* length *)
  Cstruct.set_uint8 buf 3 1;   (* HEADERS *)
  Cstruct.set_uint8 buf 4 0x04; (* END_HEADERS *)
  ...
  Cstruct.set_uint8 buf 11 16;  (* value length *)
  ...
  Cstruct.set_uint8 buf (data_off + 3) 0;  (* DATA *)
  Cstruct.set_uint8 buf (data_off + 4) 0;  (* no flags *)
  Cstruct.BE.set_uint32 buf (data_off + 5) stream_id;
  (* gRPC header: compressed=0, length=msg_len *)
  Cstruct.set_uint8 buf (data_off + 9) 0;  (* not compressed *)
  ...
  Cstruct.set_uint8 buf (trail_off + 2) 15;  (* length *)
  Cstruct.set_uint8 buf (trail_off + 3) 1;   (* HEADERS *)
  Cstruct.set_uint8 buf (trail_off + 4) 0x05; (* END_STREAM | END_HEADERS *)
  ...
  Cstruct.set_uint8 buf (trail_off + 10) 11;  (* name length *)
  Cstruct.blit_from_string "grpc-status" 0 buf (trail_off + 11) 11;
  Cstruct.set_uint8 buf (trail_off + 22) 1;   (* value length *)
...
*)
let write_echo_response_with_window_update t ~stream_id (message : Cstruct.t) ~window_increment =
  if t.closed then failwith "Connection closed";

  let msg_len = Cstruct.length message in
  let grpc_payload_len = 5 + msg_len in  (* gRPC header + message *)
  let base_len = 28 + 9 + grpc_payload_len + 24 in  (* HEADERS + DATA + TRAILERS *)
  ...
  if window_increment > 0 then begin
    ...
    Cstruct.set_uint8 buf (wu_off + 2) 4;  (* length *)
    Cstruct.set_uint8 buf (wu_off + 3) 8;  (* WINDOW_UPDATE *)
    Cstruct.set_uint8 buf (wu_off + 4) 0;  (* no flags *)
    Cstruct.BE.set_uint32 buf (wu_off + 5) 0l;  (* stream_id = 0 for connection *)
    (* Window size increment (clear reserved bit) *)
    Cstruct.BE.set_uint32 buf (wu_off + 9) (Int32.of_int (window_increment land 0x7FFFFFFF))
  end;
...
let send_settings t =
  let settings_list = [
    ...
    (Frame.Settings_id.max_concurrent_streams, Int32.of_int t.local_settings.max_concurrent_streams);
    (Frame.Settings_id.initial_window_size, Int32.of_int t.local_settings.initial_window_size);
...
let next_stream_id t ~is_client =
  ...
  if is_client && Int32.rem id 2l = 0l then
    Int32.add id 1l
  else if not is_client && Int32.rem id 2l = 1l then
    Int32.add id 1l
  else
    id
...
let client_handshake t =
  ...
  | Frame.Settings when not (Frame.Flags.is_set frame.header.flags Frame.Flags.ack) ->
    ...
    send_settings_ack t
  | _ ->
    failwith "Expected SETTINGS frame from server"
...
let server_handshake t =
  ...
  | Frame.Settings when not (Frame.Flags.is_set frame.header.flags Frame.Flags.ack) ->
    ...
    send_settings_ack t
  | _ ->
    failwith "Expected SETTINGS frame from client"
