(** gRPC Interceptor chain.
    ...
    {[
      let logging_interceptor = Interceptor.make ~name:"logging" (fun ctx next ->
        Log.info "Starting RPC: %s" ctx.method_;
        let result = next ctx in
        Log.info "Completed RPC: %s" ctx.method_;
        result
      )
...
let make ~name f : _ t = { name; intercept = (fun next ctx -> f ctx (fun c -> next c)) }

(** Create an empty context *)
let empty_context ~method_ () : context =
  { method_; metadata = []; deadline = None; extensions = Hashtbl.create 16 }
;;

(** Create context with metadata *)
let context_with_metadata ~method_ ~metadata ?deadline () : context =
  { method_; metadata; deadline; extensions = Hashtbl.create 16 }
;;
...
let deadline () : _ t =
  make ~name:"deadline" (fun ctx next ->
    if is_deadline_exceeded ctx then failwith "Deadline exceeded" else next ctx)
;;
...
let propagate_metadata (keys : string list) : _ t =
  make ~name:"propagate_metadata" (fun ctx next ->
    ...
    let filtered = List.filter (fun (k, _) -> List.mem k keys) ctx.metadata in
    ...
    {[
      let stream_logging = Interceptor.make_stream ~name:"stream_logging"
        ~on_message:(fun ctx msg ->
          Log.debug "Stream message in %s: %d bytes" ctx.method_ (String.length msg);
          msg)
        ~on_complete:(fun ctx ->
          Log.info "Stream completed: %s" ctx.method_)
        ()
    ]} *)
...
let make_stream
  ...
  : stream_t
  =
  { stream_name = name; config = { on_message; on_complete; on_error } }
;;
...
let stream_validate ~max_size () : stream_t =
  make_stream
    ~name:"stream_validate"
    ~on_message:(fun _ctx msg ->
      if String.length msg > max_size
      then
        failwith
          (Printf.sprintf "Message too large: %d > %d" (String.length msg) max_size)
      else msg)
    ()
;;
