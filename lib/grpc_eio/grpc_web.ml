...
let mode_of_content_type (content_type : string) =
  ...
  if String.starts_with ~prefix:"application/grpc-web-text" ct
  then Ok Text
  else if String.starts_with ~prefix:"application/grpc-web" ct
  then Ok Binary
    ...
    then Ok (Buffer.contents buf)
      ...
      | '%' when i + 2 < len ->
        ...
        (try
           ...
           loop (i + 3)
         with
         | _ -> Error (Printf.sprintf "Invalid percent-encoding: %%%s" hex))
...
let decode_frames_partial (buf : string) =
  ...
  let rec loop acc offset =
    if buf_len - offset < 5
    then Ok (List.rev acc, String.sub buf offset (buf_len - offset))
    else (
      let flag = Char.code buf.[offset] in
      let len = decode_u32_be buf (offset + 1) in
      if buf_len - offset < 5 + len
      then Ok (List.rev acc, String.sub buf offset (buf_len - offset))
      else (
        let payload = String.sub buf (offset + 5) len in
        let frame =
          if flag land 0x80 <> 0
          then Trailers (decode_trailers payload)
          else if flag = 0x00 || flag = 0x01
          then Message (String.sub buf offset (5 + len))
          else
...
module Base64 = struct
  ...
  let decode_quad c1 c2 c3 c4 =
    ...
    if v1 < 0 || v2 < 0
    then Error "Invalid base64 character"
    else if c3 = '=' && c4 = '='
    then (
    ...
    then (
      ...
      then Error "Invalid base64 character"
        ...
        Ok
          (Bytes.init 2 (function
             | 0 -> Char.chr (b1 land 0xFF)
             | _ -> Char.chr (b2 land 0xFF)))))
    else if c3 = '='
    then Error "Invalid base64 padding"
    else (
      let v3 = decode_table.(Char.code c3) in
  ...
  let decode (s : string) =
    ...
    if len mod 4 <> 0
    then Error "Invalid base64 length"
    else (
      let out = Buffer.create (len / 4 * 3) in
      let rec loop i =
        if i >= len
        then Ok (Buffer.contents out)
        else (
          match
            decode_quad filtered.[i] filtered.[i + 1] filtered.[i + 2] filtered.[i + 3]
          with
          | Error e -> Error e
          | Ok bytes ->
  ...
  module Stream = struct
    ...
    let decode_chunk (state : dec_state) (chunk : string) =
      let filtered =
        ...
        Buffer.contents buf
      in
      if state.finished && filtered <> ""
      then Error "Unexpected base64 data after padding"
      else (
        ...
        let rec loop i finished =
          ...
          then
            Ok
              ( Buffer.contents out
              , { carry = String.sub data full (len - full); finished } )
          else (
            let c1 = data.[i]
            and c2 = data.[i + 1]
            and c3 = data.[i + 2]
            ...
            | Ok bytes ->
              ...
              if has_padding && i + 4 < full
              then Error "Invalid base64 padding placement"
              else loop (i + 4) (finished || has_padding))
        in
    ...
    let decode_final (state : dec_state) =
      if state.carry <> ""
      then Error "Incomplete base64 quad"
