...
let calculate_backoff (policy : policy) (attempt : int) : float =
  let base =
    policy.initial_backoff *. (policy.backoff_multiplier ** float_of_int attempt)
  in
...
let sleep_seconds (seconds : float) : unit = Eio_unix.sleep seconds
...
let simple ~max_attempts : policy = { default_policy with max_attempts }
...
let conservative : policy =
  ...
  ; retryable_codes = [ Grpc_core.Status.Unavailable ]
  ; jitter = 0.1
  }
;;
