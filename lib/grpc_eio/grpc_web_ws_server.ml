...
let add_encoding_trailer ~codec trailers =
  if Grpc_core.Codec.is_identity codec || List.mem_assoc "grpc-encoding" trailers then
    trailers
  else
    ("grpc-encoding", codec.Grpc_core.Codec.name) :: trailers
...
let header_contains headers name token =
  ...
  | Some value ->
      ...
      header_tokens value
      |> List.exists (fun v -> String.lowercase_ascii v = token)
...
let read_uint64_be reader =
  ...
  logor (shift_left (b 0) 56)
    (logor (shift_left (b 1) 48)
      (logor (shift_left (b 2) 40)
        (logor (shift_left (b 3) 32)
          (logor (shift_left (b 4) 24)
            (logor (shift_left (b 5) 16)
...
let send_ws_binary flow payload =
  write_ws_frame flow ~opcode:0x2 payload

let send_ws_pong flow payload =
  write_ws_frame flow ~opcode:0xA payload

let send_ws_close flow =
  write_ws_frame flow ~opcode:0x8 ""
...
let read_request_messages ~reader ~flow ~codec ~max_size ~max_frame_size =
  ...
  let rec loop () =
    if !done_reading then Ok (List.rev !messages)
    else
      ...
      | Ws_text _ ->
          Error Grpc_core.Status.{ code = Invalid_argument; message = "Text WebSocket frames not supported"; details = None }
      | Ws_binary payload ->
...
let stream_request_messages ~sw ~reader ~flow ~codec ~max_size ~max_frame_size =
  ...
  let set_error status =
    ...
    if not !done_reading then begin
      done_reading := true;
  ...
  let rec loop () =
    if !done_reading then ()
      ...
      | Ws_binary payload ->
          ...
          (match Web.decode_frames_partial combined with
           | Error e ->
               set_error Grpc_core.Status.{ code = Invalid_argument; message = e; details = None }
           | Ok (frames, rest) ->
               pending := rest;
  ...
  Eio.Fiber.fork ~sw (fun () ->
    ...
    | Failure msg ->
        set_error Grpc_core.Status.{ code = Invalid_argument; message = msg; details = None }
    | exn ->
        set_error Grpc_core.Status.{
          code = Unknown;
...
let send_unary_response ~flow ~codec ~resp_body ~trailers =
  let trailers =
    if List.mem_assoc "grpc-status" trailers then trailers else ("grpc-status", "0") :: trailers
  in
...
let send_streaming_response ~flow ~clock ~timeout ~codec ~response_stream ?error_promise () =
  ...
  let rec write_messages () =
    if deadline_exceeded () then
      ...
      let wait_next () =
        ...
        | None ->
            `Msg (Grpc_stream.take response_stream)
        | Some promise ->
            Eio.Fiber.first
        ...
        try
          ...
          | Some d ->
              ...
              if remaining <= 0.0 then (
                ...
                Some (Eio.Time.with_timeout_exn clock remaining wait_next)
        with
        | End_of_file -> None
        | Eio.Time.Timeout ->
            error_status := Some deadline_status;
      ...
      | Some (`Msg msg) ->
          (match Grpc_core.Message.encode ~codec msg with
           | Error e ->
               error_status := Some Grpc_core.Status.{ code = Internal; message = e; details = None }
           | Ok frame ->
...
let handle_grpc_websocket ~sw ~clock ~server ~config ~flow ~reader
    ~(path : string)
  ...
  let read_result =
    read_request_messages ~reader ~flow ~codec:request_codec ~max_size
                ...
                (match method_def.handler with
                 | `ServerStreaming handler ->
                     ...
                     send_streaming_response ~flow ~clock ~timeout:timeout_seconds
                       ~codec:response_codec ~response_stream ();
                     send_ws_close flow
                 | _ ->
                     let status = Grpc_core.Status.{
                       code = Internal;
           ...
           (match method_def.handler with
            | `ClientStreaming handler ->
                let request_stream, request_error, _error_promise =
                  stream_request_messages ~sw ~reader ~flow ~codec:request_codec
                    ~max_size ~max_frame_size:config.max_frame_size
                in
                let timeout_seconds = timeout_seconds_of_metadata server metadata in
                let response =
                 ...
                 | None ->
                     (match response with
                      | Ok resp_body ->
                          ...
                          (match resp with
                           | Ok frame ->
                               send_unary_response ~flow ~codec:response_codec ~resp_body:frame ~trailers:[];
                               send_ws_close flow
                           | Error status ->
                               send_grpc_error ~flow status;
                               send_ws_close flow)
       ...
       | Service.BidiStreaming ->
           (match method_def.handler with
            | `Bidi handler ->
                ...
                let timeout_seconds = timeout_seconds_of_metadata server metadata in
                send_streaming_response ~flow ~clock ~timeout:timeout_seconds
                  ~codec:response_codec ~response_stream ~error_promise ();
                send_ws_close flow
            | _ ->
                let status = Grpc_core.Status.{
                  code = Internal;
...
let serve ?config ~sw ~env (server : Server.t) =
  ...
  Eio.traceln "gRPC-Web WebSocket gateway on %a" Eio.Net.Sockaddr.pp config.addr;
  ...
  let handle_connection flow _addr =
    ...
    try
      ...
      let req =
        Http1.read_request reader ~max_header_size:16384 ~max_body_size:0
      in
      if String.uppercase_ascii req.meth = "GET" then
          ...
          let path =
            ...
            | p :: _ -> p
          in
          if not upgrade_ok || not conn_ok || not version_ok || key = None then
            ...
            | Ok (_service, method_def, _) ->
                ...
                | Ok request_codec, Ok response_codec ->
                    ...
                    let requested_proto = Http1.header_value req.headers "sec-websocket-protocol" in
                    let selected_proto =
                      select_subprotocol ~supported:config.subprotocols ~requested:requested_proto
                    in
                    let headers = [
                      ("upgrade", "websocket");
                      ("connection", "Upgrade");
                    ...
                    let headers = match selected_proto with
                      | None -> headers
                      | Some proto -> ("sec-websocket-protocol", proto) :: headers
                    in
                    Http1.write_response_headers flow ~status:101 ~reason:"Switching Protocols" ~headers;
                    upgraded := true;
                    handle_grpc_websocket ~sw ~clock:(Eio.Stdenv.clock env) ~server ~config ~flow ~reader
                      ~path ~metadata ~method_def ~request_codec ~response_codec
      else
        send_plain ~flow ~status:405 ~reason:"Method Not Allowed"
    with
    | End_of_file -> ()
    | Failure msg ->
        if !upgraded then send_ws_close flow
  ...
  while true do
    Eio.Net.accept_fork socket ~sw
      ~on_error:(fun exn -> Eio.traceln "gRPC-Web WebSocket connection error: %s" (Printexc.to_string exn))
      accept_tls
  done
