...
let supported_encodings (server : t) : string =
  server.config.codecs
  |> List.map (fun c -> c.Grpc_core.Codec.name)
  |> String.concat ", "

let create ?(config = default_config) () : t =
  { config; services = Hashtbl.create 16; interceptors = []; metrics = None; running = false }
...
let response_codec_of_metadata (server : t) (metadata : (string * string) list)
  ...
  | Some accepted ->
      ...
      (match List.find_opt (fun c ->
       ...
       | Some codec -> Ok codec
       | None ->
           if List.mem "identity" accepted_list then
             Ok Grpc_core.Codec.identity
           else
             Error Grpc_core.Status.{
               code = Unimplemented;
...
let timeout_seconds_of_metadata (server : t) (metadata : (string * string) list) : float option =
...
let lookup_method (server : t) (path : string)
  ...
  | None ->
      Error Grpc_core.Status.{
        code = Unimplemented;
        message = Printf.sprintf "Invalid path format: '%s'. Expected: /package.Service/Method" path;
        details = None;
      }
  | Some (service_name, method_name) ->
      ...
      | None ->
          ...
          Error Grpc_core.Status.{
            code = Unimplemented;
            message = Printf.sprintf "Service '%s' not found. Available: [%s]" service_name available;
            details = None;
          }
      | Some service ->
          ...
          | None ->
              Error Grpc_core.Status.{
                code = Unimplemented;
                message = Printf.sprintf "Method '%s' not found in service '%s'. Check your .proto file." method_name service_name;
                details = None;
              }
          | Some method_def ->
              Ok (service, method_def, Grpc_core.Codec.identity)
...
let decode_request ~codec body : (string, Grpc_core.Status.t) result =
  ...
  | Error e ->
      let hint = if String.length body > 4 * 1024 * 1024
...
let invoke_handler (server : t) ~clock ~path ~metadata ~response_codec ~method_def ~request_data
  ...
  let base_handler _ctx =
    ...
    | `Unary handler ->
        ...
        (match Grpc_core.Message.encode ~codec:response_codec response with
         ...
         | Error e -> failwith e)  (* Caught by outer exception handler *)
  ...
  in
  try
    let result = match timeout_seconds with
      | None ->
  ...
  | Eio.Time.Timeout ->
      let timeout_str = match timeout_seconds with
  ...
  | Failure msg ->
      Eio.traceln "gRPC handler failure: %s" msg;
      Error Grpc_core.Status.{ code = Internal; message = "Internal server error"; details = None }
  (* All other exceptions - log details but don't expose to client *)
  | exn ->
      Eio.traceln "gRPC unexpected error: %s\n%s"
...
let handle_request (server : t) ~clock ~path ~metadata ~body ~request_codec ~response_codec
  : (string * (string * string) list * Grpc_core.Codec.t, Grpc_core.Status.t) result =
...
let handle_decoded_request (server : t) ~clock ~path ~metadata ~request_data ~response_codec ~method_def
  : (string * (string * string) list * Grpc_core.Codec.t, Grpc_core.Status.t) result =
  invoke_handler server ~clock ~path ~metadata ~response_codec ~method_def ~request_data
...
let request_handler ~sw ~clock (server : t) reqd =
  ...
  let content_type = Headers.get request.headers "content-type" |> Option.value ~default:"" in
  if not (String.starts_with ~prefix:"application/grpc" content_type) then begin
    Reqd.respond_with_string reqd (Response.create `Unsupported_media_type) "Invalid content-type"
  end else begin
    match lookup_method server path with
    | Error status -> Http2_handler.send_error ~reqd ~encodings status
    | Ok (_service, method_def, _) ->
        let metadata = Headers.fold ~f:(fun k v acc -> (String.lowercase_ascii k, v) :: acc) ~init:[] request.headers in
        ...
        | Ok request_codec, Ok response_codec ->
            ...
            | `Unary _ ->
                ...
                Option.iter (fun m -> Metrics.record_call_start m ~method_:path) server.metrics;
                ...
                let rec read_body () =
                  Body.Reader.schedule_read body
                    ~on_eof:(fun () ->
                      ...
                      match handle_request server ~clock ~path ~metadata ~body:full_body ~request_codec ~response_codec with
                      | Ok (resp_body, trailers, resp_codec) ->
                          Option.iter
                            (fun m ->
                              Metrics.record_call_end m
                                ~method_:path
                                ...
                                ())
                            server.metrics;
                          Http2_handler.send_response ~reqd ~encodings ~codec:resp_codec ~body:resp_body ~trailers
                      | Error status ->
                          Option.iter
                            (fun m ->
                              Metrics.record_call_end m
                ...
                let rec read_body () =
                  Body.Reader.schedule_read body
                    ~on_eof:(fun () ->
                      ...
                      | Error e ->
                          Http2_handler.send_error ~reqd ~encodings
...
let serve ~sw ~env (server : t) : unit =
  ...
  let tls_server_config = match server.config.tls with
    | Some tls ->
        (* RNG initialization should be done at app level with Mirage_crypto_rng_eio.run *)
        let config = Tls_config.load tls in
        Eio.traceln "🔒 TLS enabled (ALPN: h2)";
        Some config
    | None ->
        Eio.traceln "ℹ️  Running without TLS (plaintext h2c)";
        None
  in

  Eio.traceln "gRPC server on %s:%d" server.config.host server.config.port;
  server.running <- true;
  ...
  while server.running do
    Eio.Net.accept_fork socket ~sw
      ~on_error:(fun exn -> Eio.traceln "Connection error: %s" (Printexc.to_string exn))
      handle_connection
  done

let shutdown (server : t) : unit =
  Eio.traceln "Shutting down gRPC server...";
