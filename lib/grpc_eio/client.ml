...
module Credentials = struct
  ...
  let mtls ~ca_file ~cert_file ~key_file () : channel_credentials =
    Tls { ca_file = Some ca_file; cert_file = Some cert_file; key_file = Some key_file; verify_peer = true }
  ...
  let with_token ~token ?(tls : tls_config option) () : channel_credentials =
    let transport = match tls with
      | Some cfg -> Tls cfg
      | None -> Tls { ca_file = None; cert_file = None; key_file = None; verify_peer = true }
    in
  ...
  let requires_tls (creds : channel_credentials) : bool =
    ...
    | WithCallCredentials { transport; _ } ->
      (match transport with
       | Insecure -> false
       | Tls _ -> true
       | WithCallCredentials _ -> true)  (* Nested: assume TLS *)
...
let default_keepalive : keepalive_config = {
  time = 7200.0;           (* 2 hours - gRPC default *)
...
let default_config ~target : config = {
  ...
  timeout = Some 30.0;
  metadata = [];
  tls = None;  (* Legacy: Auto-detect from scheme *)
  credentials = None;  (* Use Credentials module for new code *)
...
let config_with_credentials ~target ~credentials : config = {
  ...
  timeout = Some 30.0;
  metadata = [];
  tls = Credentials.to_tls_config credentials;  (* For backward compatibility *)
...
let tls_mtls ~ca_file ~cert_file ~key_file : tls_config =
  { ca_file = Some ca_file; cert_file = Some cert_file; key_file = Some key_file; verify_peer = true }
...
let parse_target (target : string) : (string * string * int) option =
  try
    ...
    let port = Uri.port uri |> Option.value ~default:(if scheme = "https" then 443 else 80) in
...
let build_tls_client_config ~host (tls_cfg : tls_config option) : Tls.Config.client option =
  ...
  let authenticator = match tls_cfg with
  ...
  let certificates = match tls_cfg with
    | Some { cert_file = Some cert_file; key_file = Some key_file; _ } ->
        ...
        (match X509.Certificate.decode_pem_multiple cert_pem,
               X509.Private_key.decode_pem key_pem with
...
let connect ?(config : config option) ~sw:_ ~env:_ (target : string) : t =
  ...
  | Some (scheme, host, port) ->
      ...
      let effective_tls = match config.credentials with
        | Some creds -> Credentials.to_tls_config creds
        | None -> config.tls
      in
      (* Extract call credentials headers *)
      let call_credentials_headers = match config.credentials with
      ...
      let tls_client_config =
        if scheme = "https" then
          build_tls_client_config ~host effective_tls
        else
          None
      in
      { config;
...
let call_unary
  ...
  let effective_timeout = match deadline with
    | Some d -> Some d
    | None -> client.config.timeout
  in
  let headers = match effective_timeout with
    | None -> headers
    | Some secs ->
  ...
  let get_or_create_connection () =
    ...
    | None ->
        ...
        let conn = match client.tls_client_config with
          | Some tls_config ->
              let tls_flow = Tls_eio.client_of_flow tls_config socket in
              Flow_handler.create_client_connection ~sw
                ~error_handler:(fun _ -> ()) tls_flow
          | None ->
              let h2_client = H2_eio.Client.create_connection ~sw
                ~error_handler:(fun _ -> ()) socket in
    ...
    let rec read () =
      H2.Body.Reader.schedule_read body
        ~on_eof:(fun () ->
          ...
          | Some code ->
              Eio.Promise.resolve response_resolver (Error Grpc_core.Status.{
                  code = Grpc_core.Status.code_of_int code;
                  message;
                  details;
                })
          | None ->
              ...
              let msg =
                Printf.sprintf "missing grpc-status, inferred from HTTP status code %d" http_code
              in
              Eio.Promise.resolve response_resolver (Error Grpc_core.Status.{
                  code;
                  message = msg;
  ...
  let request_body =
    H2.Client_connection.request connection
  ...
  | Ok body ->
      ...
      | Error e -> Error Grpc_core.Status.{
          code = Internal;
...
let call_server_streaming
  ...
  Eio.Fiber.fork ~sw (fun () ->
    ...
    let framed_request = match Grpc_core.Message.encode ~codec request with
      | Ok f -> f
      | Error e ->
          Grpc_stream.add stream (Error Grpc_core.Status.{
            code = Internal;
            message = Printf.sprintf "Request encoding failed: %s" e;
            details = None;
          });
        ...
        H2.Body.Reader.schedule_read body
          ~on_eof:(fun () ->
            ...
            let status =
              ...
              | Some code ->
                  Grpc_core.Status.{ code = Grpc_core.Status.code_of_int code; message; details }
              | None ->
                  let http_code = H2.Status.to_code response.status in
                  let code = map_http_status_code http_code in
                  let msg =
          ...
          ~on_read:(fun bs ~off ~len ->
            ...
            | Error (Grpc_core.Message.Oversized len) ->
                Grpc_stream.add stream (Error Grpc_core.Status.{
                  ...
                  message = Printf.sprintf "Server sent oversized message: %d bytes" len;
                  details = None;
                });
                Grpc_stream.close stream
    ...
    let connection : H2.Client_connection.t = match client.tls_client_config with
      | Some tls_config ->
          let tls_flow = Tls_eio.client_of_flow tls_config socket in
    ...
    let rec write_loop () =
      ...
      | req ->
          (match Grpc_core.Message.encode ~codec req with
           ...
           | Error _ -> ());  (* Skip malformed messages *)
          write_loop ()
...
let call_bidi
  ...
  Eio.Fiber.fork ~sw (fun () ->
    ...
    let socket = match addrs with
      | [] ->
          Grpc_stream.add responses (Error Grpc_core.Status.{
            code = Unavailable;
...
let ping
    ...
    let msg = match error with
      ...
      | `Protocol_error (code, msg) ->
          Printf.sprintf "Protocol error %s: %s"
            (H2.Error_code.to_string code) msg
    in
    Eio.Promise.resolve ping_resolver (Error Grpc_core.Status.{
        code = Unavailable;
        message = msg;
        details = None;
      })
  ...
  H2.Client_connection.ping connection (function
    ...
    | Error `EOF -> Eio.Promise.resolve ping_resolver (Error Grpc_core.Status.{
        code = Unavailable;
  ...
  let timeout_secs = match client.config.keepalive with
    | Some ka -> ka.timeout
  ...
  Eio.Fiber.first
    (fun () ->
      result := Some (Eio.Promise.await ping_promise))
    (fun () ->
      Eio.Time.sleep clock timeout_secs;
      result := Some (Error Grpc_core.Status.{
  ...
  match !result with
  | Some r -> r
  | None -> Error Grpc_core.Status.{
      code = Internal;
      message = "Unexpected: no result";
      details = None;
    }
