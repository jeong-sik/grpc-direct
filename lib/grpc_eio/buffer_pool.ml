...
let size_classes =
  [| 256
   ; (* Small messages, headers *)
     1024
   ; (* Typical unary request *)
     4096
   ; (* Medium messages *)
     16384
   ; (* Large messages *)
...
let size_class_index requested_size =
  let rec find i =
    if i >= Array.length size_classes
    then None
    else if size_classes.(i) >= requested_size
    then Some i
    else find (i + 1)
  in
  find 0
...
module SizeClassPool = struct
  ...
  let create size = { size; buffers = []; count = 0; hits = 0; misses = 0 }
  ...
  let release pool buf =
    if Bytes.length buf = pool.size && pool.count < max_pooled
    then (
...
type t = { pools : SizeClassPool.t array }

let create () = { pools = Array.map SizeClassPool.create size_classes }
...
let release buf =
  ...
  match size_class_index size with
  | Some idx when size_classes.(idx) = size -> SizeClassPool.release pool.pools.(idx) buf
...
let slice_to_string slice = Bytes.sub_string slice.buffer slice.off slice.len

let slice_sub slice ~off ~len =
  if off < 0 || len < 0 || off + len > slice.len
  then invalid_arg "Buffer_pool.slice_sub: out of bounds";
  { slice with off = slice.off + off; len }
;;

let slice_release slice = if slice.pooled then release slice.buffer
...
module Builder = struct
  ...
  let ensure_capacity builder additional =
    ...
    if needed > slice_length builder.slice
    then (
      let new_size = max (slice_length builder.slice * 2) needed in
      let new_slice = slice_create ~pooled:true new_size in
      Bytes.blit builder.slice.buffer builder.slice.off new_slice.buffer 0 builder.pos;
  ...
  let contents builder = Bytes.sub_string builder.slice.buffer 0 builder.pos
  let to_slice builder = { builder.slice with len = builder.pos }
  let release builder = slice_release builder.slice
end
