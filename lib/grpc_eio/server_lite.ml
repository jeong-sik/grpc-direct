...
let parse_path path : (string * string) option =
  if String.length path < 4 || path.[0] <> '/' then None
  else
...
let handle_connection server flow =
  ...
  let send_error stream_id code message =
    ...
    let trailers = [
      ("grpc-status", string_of_int code);
      ("grpc-message", percent_encode message);
    ] in
    let encoded_trailers = Hpack.encode hpack_encoder trailers in

    let header_frames =
      Frame.make_headers_fragmented ~stream_id ~end_stream:false
        ~max_frame_size:(Connection.peer_max_frame_size conn) encoded_headers
    in
    let trailer_frames =
      Frame.make_headers_fragmented ~stream_id ~end_stream:true
  ...
  let send_grpc_response stream_id (response_body : string) =
    ...
    if Flow_control.can_send flow_control stream_id data_len then begin
      ...
      send_error stream_id 8 "Flow control window exhausted"
  in
      ...
      | `Unary handler ->
          (try
              ...
              if Cstruct.length remaining <> 0 then
                send_error stream_id 3 "Invalid gRPC framing: extra bytes after message"
              else begin
            ...
            with exn ->
              send_error stream_id 13 (Printf.sprintf "Internal error: %s" (Printexc.to_string exn)))
        | _ ->
            send_error stream_id 12 "Streaming methods not yet supported in Server_lite"
  in
    ...
    while true do
      ...
      | Frame.Headers ->
          ...
          if not headers_ok then begin
            let rst = Frame.make_rst_stream ~stream_id
      ...
      | Frame.Ping ->
          Connection.send_ping conn ~ack:true frame.payload

      | Frame.WindowUpdate ->
          if Cstruct.length frame.payload <> 4 then
          ...
          if increment = 0 then begin
            ...
            Flow_control.update flow_control stream_id increment

      | Frame.GoAway ->
          raise Exit (* Client initiated shutdown *)
      ...
      | _ ->
          () (* Ignore unknown frames per RFC 7540 §4.1 *)
    done
  with
