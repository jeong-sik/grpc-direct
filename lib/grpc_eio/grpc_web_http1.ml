...
let read_line reader =
  Eio.Buf_read.line reader |> strip_cr
...
let parse_chunk_size line =
  ...
  if trimmed = "" then None
    ...
    let hex = match String.index_opt trimmed ';' with
      | None -> trimmed
...
let read_body reader ~max_body_size headers =
  ...
  | Some v when List.mem "chunked" (String.split_on_char ',' v |> List.map String.trim |> List.map String.lowercase_ascii) ->
      ...
      let rec loop () =
        ...
        | Some n ->
            ...
            ignore (read_line reader); (* consume CRLF *)
...
let finish_chunked flow =
  Eio.Flow.write flow [Cstruct.of_string "0\r\n\r\n"]
