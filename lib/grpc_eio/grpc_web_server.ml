...
let preflight_headers (cors : cors_config) =
  ...
  let headers =
    ("access-control-allow-methods", String.concat ", " cors.allow_methods) :: headers
  in
  let headers =
    ("access-control-allow-headers", String.concat ", " cors.allow_headers) :: headers
  in
  let headers =
    match cors.max_age with
    | None -> headers
...
let grpc_web_headers ~cors ~mode ~encodings ~codec =
  let headers =
    [ "content-type", Web.content_type mode; "grpc-accept-encoding", encodings ]
  in
  let headers =
...
let decode_messages ~codec ~max_size frames =
  let rec loop acc = function
    ...
    | Web.Trailers _ :: rest -> loop acc rest
  in
  loop [] frames
;;
...
let send_streaming_response
  ...
  let send_frame frame =
    match mode with
    | Web.Binary -> Http1.write_chunk flow frame
    | Web.Text ->
  ...
  let rec write_messages () =
    ...
    then error_status := Some deadline_status
      ...
      let next =
        match deadline with
        | None ->
          (try Some (Grpc_stream.take response_stream) with
           | End_of_file -> None)
        | Some d ->
...
let handle_grpc_web ~sw:_ ~clock ~server ~config ~flow (req : Http1.request) =
  ...
  | Error _ ->
    ...
    send_plain ~flow ~status:415 ~reason:"Unsupported Media Type" ~headers ~body:""
  | Ok mode ->
    (match Server.lookup_method server req.target with
     | Error status -> send_grpc_web_error ~flow ~cors:config.cors ~mode ~encodings status
                      ...
                      (match method_def.handler with
                       | `ClientStreaming handler ->
                         ...
                         (match response with
                          | Ok resp_body ->
                            (match
                               Grpc_core.Message.encode ~codec:response_codec resp_body
                             with
                             | Ok framed ->
                               send_unary_response
                                 ~flow
...
let serve ?config ~sw ~env (server : Server.t) =
  ...
  Eio.traceln "gRPC-Web server on %s" (Format.asprintf "%a" Eio.Net.Sockaddr.pp config.addr);
  let handle_connection flow _addr =
    try
      ...
      | "POST" -> handle_grpc_web ~sw ~clock ~server ~config ~flow req
      | _ ->
        let headers = cors_headers config.cors in
        let headers = ("content-length", "0") :: headers in
        send_plain ~flow ~status:405 ~reason:"Method Not Allowed" ~headers ~body:""
    with
    | End_of_file -> ()
    | Failure msg ->
      let headers = cors_headers config.cors in
