...
let connect ?config ~env:_ (target : string) =
  let config = Option.value ~default:(default_config ~target) config in
  match parse_target config.target with
  | None -> failwith (Printf.sprintf "Invalid target URI: %s" config.target)
  | Some (scheme, host, port) ->
      let tls_client_config =
        if scheme = "https" then build_tls_client_config ~host config.tls else None
      in
      { config; scheme; host; port; tls_client_config; closed = false }
...
let connect_flow ~sw ~net (client : t) : Eio.Flow.two_way_ty Eio.Std.r =
  ...
  | "https" ->
      (match client.tls_client_config with
       | Some cfg ->
           (Tls_eio.client_of_flow cfg socket
            :> Eio.Flow.two_way_ty Eio.Std.r)
       | None -> failwith "TLS config missing for https")
  | _ ->
...
let call_unary ~sw ~env ?(deadline : float option) (client : t)
    ~(service : string) ~(method_ : string) ~(request : string) =
  if client.closed then failwith "Client is closed";
  ...
  | Ok framed_request ->
      ...
      let accept_enc =
        if Grpc_core.Codec.is_identity codec then "identity"
      ...
      let flow = connect_flow ~sw ~net client in
      Http1.write_request flow ~meth:"POST" ~target:path ~headers ~body;

      let reader = Eio.Buf_read.of_flow ~max_size:(8 * 1024 * 1024) flow in
      let resp = Http1.read_response reader ~max_header_size:16384 ~max_body_size:(64 * 1024 * 1024) in
      if resp.status_code <> 200 then
        Error Grpc_core.Status.{ code = Unknown; message = "HTTP error"; details = None }
      else
        let mode =
        ...
        | Ok body ->
            ...
            | Ok frames ->
                let trailers =
                  frames
                  |> List.filter_map (function Web.Trailers t -> Some t | _ -> None)
                  |> List.concat
                in
                let status = header_list_to_status trailers in
                if not (Grpc_core.Status.is_ok status) then Error status
                else
                  let resp_codec =
                    match Http1.header_value resp.headers "grpc-encoding" with
                    | None -> Grpc_core.Codec.identity
                    | Some enc ->
                        (match Grpc_core.Codec.find_by_name ~supported:(supported_codecs client) enc with
                         | Some c -> c
                         | None -> Grpc_core.Codec.identity)
                  in
                  decode_response_message ~codec:resp_codec frames

let call_server_streaming ~sw ~env ?(deadline : float option) (client : t)
    ~(service : string) ~(method_ : string) ~(request : string) =
  ...
  Eio.Fiber.fork ~sw (fun () ->
    ...
    let framed_request = match Grpc_core.Message.encode ~codec request with
      | Ok f -> f
      | Error e ->
          Grpc_stream.add stream (Error Grpc_core.Status.{ code = Internal; message = e; details = None });
    ...
    let reader = Eio.Buf_read.of_flow ~max_size:(8 * 1024 * 1024) flow in
    let (_ver, status_code, _reason, headers) = Http1.read_response_head reader ~max_header_size:16384 in
    if status_code <> 200 then begin
      Grpc_stream.add stream (Error Grpc_core.Status.{ code = Unknown; message = "HTTP error"; details = None });
      Grpc_stream.close stream;
      raise Exit
    end;
    ...
    let handle_binary chunk =
      ...
      | Error e ->
          Grpc_stream.add stream (Error Grpc_core.Status.{ code = Invalid_argument; message = e; details = None });
          done_stream := true;
          Grpc_stream.close stream
      | Ok (frames, remaining) ->
          buffer := remaining;
    ...
    Http1.read_body_chunks reader ~max_body_size:(64 * 1024 * 1024) headers ~on_chunk:handle_chunk;
    (match resp_mode with
     | Web.Binary -> ()
     | Web.Text ->
...
let call_client_streaming ~sw ~env ?(deadline : float option) (client : t)
    ~(service : string) ~(method_ : string) ~(requests : string Grpc_stream.t) =
  if client.closed then failwith "Client is closed";
  let net = Eio.Stdenv.net env in
  let path = Printf.sprintf "/%s/%s" service method_ in
  let codec = client.config.codec in
...
let call_bidi ~sw ~env ?(deadline : float option) (client : t)
    ~(service : string) ~(method_ : string) ~(requests : string Grpc_stream.t) =
  let stream = Grpc_stream.create 16 in
  Eio.Fiber.fork ~sw (fun () ->
    let net = Eio.Stdenv.net env in
