...
let connect_flow ~sw ~net (client : t) : Eio.Flow.two_way_ty Eio.Std.r =
  ...
  | "https" ->
    (match client.tls_client_config with
     | Some cfg -> (Tls_eio.client_of_flow cfg socket :> Eio.Flow.two_way_ty Eio.Std.r)
     | None -> failwith "TLS config missing for https")
  | _ -> (socket :> Eio.Flow.two_way_ty Eio.Std.r)
;;
...
let call_unary
  ...
  | Ok framed_request ->
    ...
    let resp =
      Http1.read_response reader ~max_header_size:16384 ~max_body_size:(64 * 1024 * 1024)
    in
    if resp.status_code <> 200
    then Error Grpc_core.Status.{ code = Unknown; message = "HTTP error"; details = None }
    else (
      let mode =
