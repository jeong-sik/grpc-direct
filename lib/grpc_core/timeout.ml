  ...
  then None
    ...
    | Some mult ->
      (try
         ...
         if value <= 0L then None else Some (Int64.mul value mult)
       with
       | Failure _ -> None))
;;
...
let to_string t =
  if Int64.rem t h = 0L
  then Printf.sprintf "%Ld%c" (Int64.div t h) 'H'
  else if Int64.rem t m = 0L
  then Printf.sprintf "%Ld%c" (Int64.div t m) 'M'
  else if Int64.rem t s = 0L
  then Printf.sprintf "%Ld%c" (Int64.div t s) 'S'
  else if Int64.rem t ms = 0L
  then Printf.sprintf "%Ld%c" (Int64.div t ms) 'm'
  else if Int64.rem t us = 0L
  then Printf.sprintf "%Ld%c" (Int64.div t us) 'u'
