...
let decode ?(max_size = default_max_size) ~codec frame =
  if String.length frame < 5 then
    Error "Frame too short"
  else
    let compressed = frame.[0] = '\x01' in
    let len =
      (Char.code frame.[1] lsl 24) lor
      (Char.code frame.[2] lsl 16) lor
      (Char.code frame.[3] lsl 8) lor
      Char.code frame.[4]
    in
    (* Security: Validate message size before allocation to prevent DoS *)
    if len > max_size then
      Error (Printf.sprintf "Message too large: %d bytes (max: %d)" len max_size)
    else if String.length frame < 5 + len then
      Error "Frame incomplete"
    else if compressed && Codec.is_identity codec then
      Error "Compressed flag set but grpc-encoding not set"
    else
      let data = String.sub frame 5 len in
      if compressed then
        codec.Codec.decoder data
      else
        Ok data
...
let extract_all ?(max_size = default_max_size) ~codec buf
  ...
  let rec loop acc remaining =
    if String.length remaining < 5 then
      Ok (List.rev acc, remaining)
    else
      let len =
        (Char.code remaining.[1] lsl 24) lor
        (Char.code remaining.[2] lsl 16) lor
        (Char.code remaining.[3] lsl 8) lor
        Char.code remaining.[4]
      in
      (* Security: Reject oversized messages with explicit error *)
      if len > max_size then
        Error (Oversized len)
      else if String.length remaining < 5 + len then
        Ok (List.rev acc, remaining)  (* Incomplete frame, wait for more data *)
      else
