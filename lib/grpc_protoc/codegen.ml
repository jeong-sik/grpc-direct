  ...
  let handler_registrations =
    List.map
      (fun m ->
         ...
         | Unary ->
           Printf.sprintf
             {|      |> Grpc_eio.Service.add_unary "%s" (fun bytes ->
            let decoder = Pbrt.Decoder.of_string bytes in
         ...
         | ServerStreaming ->
           Printf.sprintf
             {|      |> Grpc_eio.Service.add_server_streaming "%s" (fun bytes ->
            let decoder = Pbrt.Decoder.of_string bytes in
            let request = %s.from_proto decoder in
            Impl.%s request
          )|}
         ...
         | ClientStreaming ->
           Printf.sprintf
             {|      |> Grpc_eio.Service.add_client_streaming "%s" (fun request_stream ->
         ...
         | BidiStreaming ->
           Printf.sprintf
             {|      |> Grpc_eio.Service.add_bidi_streaming "%s" (fun request_stream ->
            Impl.%s request_stream
          )|}
             m.name
             (to_snake_case m.name))
...
let generate_service (service : service_def) : string =
  ...
  Printf.sprintf
    {|(** Generated gRPC stubs for %s.%s

    This file was auto-generated by protoc-gen-grpc-direct.
...
let parse_service_description (input : string) : service_def option =
  ...
  List.iter
    (fun line ->
       if String.starts_with ~prefix:"package:" line
  ...
  then None
    ...
    Some { package = !package; service_name = !service_name; methods = List.rev !methods }
;;
