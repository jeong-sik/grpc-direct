...
module InferRequest = struct
  let create ?(params = []) input =
    let param_str = List.map (fun (k, v) -> Printf.sprintf "%s=%s" k v) params
      |> String.concat "|" in
    if param_str = "" then input
    else Printf.sprintf "%s|%s" input param_str
end
...
module InferResponse = struct
  ...
  let of_bytes bytes =
    ...
    match parts with
    | output :: rest ->
        let latency_ms = List.find_map (fun s ->
          if String.length s > 8 && String.sub s 0 8 = "latency:" then
            Some (float_of_string (String.sub s 8 (String.length s - 8)))
          else None
        ) rest |> Option.value ~default:0.0 in
        let model_version = List.find_map (fun s ->
          if String.length s > 8 && String.sub s 0 8 = "version:" then
            Some (String.sub s 8 (String.length s - 8))
          else None
        ) rest |> Option.value ~default:"unknown" in
        { output; latency_ms; model_version }
    | _ ->
        { output = bytes; latency_ms = 0.0; model_version = "unknown" }
end
...
let infer_with_retry client ~sw ~env ~max_retries input =
  let rec attempt n =
    if n > max_retries then
      ...
      let request = InferRequest.create
      ...
      with
      | Ok response_bytes ->
          Ok (InferResponse.of_bytes response_bytes)
      | Error status when Grpc_core.Status.(status.code = Unavailable) ->
          (* Retry on transient errors *)
          attempt (n + 1)
      | Error status ->
          Error status.Grpc_core.Status.message
    end
  in
  attempt 0
...
let () =
  ...
  let retry_policy = Grpc_eio.Retry.{
    default_policy with
    max_attempts = 3;
    initial_backoff = 0.1;
    retryable_codes = [Grpc_core.Status.Unavailable; Grpc_core.Status.Resource_exhausted];
  } in

  (* Connect with timeout and retry interceptor *)
  let config = Grpc_eio.Client.{
    (default_config ~target:"http://127.0.0.1:50051") with
    timeout = Some 10.0;
  } in
  let client = Grpc_eio.Client.connect ~config ~sw ~env "http://127.0.0.1:50051"
    |> Grpc_eio.Client.with_interceptor (Grpc_eio.Retry.interceptor ~policy:retry_policy ())
  in
  Printf.printf "📡 Connected to inference server (with retry policy)\n\n%!";

  (* Health check first *)
  if not (check_health client ~sw ~env) then begin
