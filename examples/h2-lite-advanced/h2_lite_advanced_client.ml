...
let () =
  ...
  let on_push (push : H2_lite.push_response) =
    ...
    Log.info "PUSH stream=%ld body=%s" push.promised_stream_id (Cstruct.to_string body)
  in
  Eio_main.run
  @@ fun env ->
  Eio.Switch.run
  ...
  let rec read_response () =
    ...
    (match frame.Frame.header.frame_type with
     ...
     | Frame.Data -> Buffer.add_string response_buffer (Cstruct.to_string frame.payload)
  ...
  | Some "0" ->
    ...
    in
    Log.info "OK: %s" (Cstruct.to_string resp_body)
  | Some code ->
    let msg = Option.value ~default:"Unknown error" !status_message in
    Log.error "gRPC error: %s (%s)" code msg
  | None -> Log.warn "Missing grpc-status"
;;
