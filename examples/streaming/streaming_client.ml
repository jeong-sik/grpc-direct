...
let test_server_streaming ~sw ~env client =
  ...
  let rec read_all count =
    ...
    | Error status ->
      ...
      then Printf.printf "✅ Stream complete: %d messages\n%!" count
      else Printf.printf "❌ Stream error: %s\n%!" status.message
    | exception End_of_file -> Printf.printf "✅ Stream ended: %d messages\n%!" count
  in
  read_all 0
;;
...
let test_client_streaming ~sw ~env client =
  ...
  match
    Grpc_eio.Client.call_client_streaming
      ~sw
      ~env
      client
      ~service:"streaming.Echo"
...
let take_with_timeout ~clock ~timeout stream =
  try
    Msg (Eio.Time.with_timeout_exn clock timeout (fun () -> Grpc_eio.Stream.take stream))
  with
...
let test_bidi_streaming ~sw ~env ~clock client =
  ...
  let rec read_all count =
    if count >= expected
    then Printf.printf "✅ Bidi received %d/%d messages\n%!" count expected
    else (
      ...
      | Msg (Error status) ->
        if status.Grpc_core.Status.code = Grpc_core.Status.OK
        then Printf.printf "✅ Bidi stream complete: %d messages\n%!" count
        else Printf.printf "❌ Stream error: %s\n%!" status.message
      | Timeout ->
        Printf.printf "⏱️  Bidi response timeout; stopping after %d/%d\n%!" count expected
      | Eof -> Printf.printf "✅ Bidi stream ended: %d messages\n%!" count)
  in
  read_all 0;
