...
module EchoService = struct
  ...
  let echo_expand bytes =
    let msg = Message.of_bytes bytes in
    let count = max 1 (min msg.seq 10) in  (* Send 1-10 messages *)
    let stream = Grpc_eio.Stream.create 16 in
    for i = 1 to count do
      let response = { Message.seq = i; content = Printf.sprintf "Response %d: %s" i msg.content } in
  ...
  let echo_collect request_stream =
    ...
    in
    drain ();
    let summary = Printf.sprintf "Received %d messages: [%s]"
      !count (String.concat ", " (List.rev !contents))
    in
