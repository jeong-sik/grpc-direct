...
module EmbedRequest = struct
  ...
  let to_bytes t =
    Printf.sprintf "%s|%s" t.model (String.concat "|" t.texts)
end
...
module EmbedResponse = struct
  ...
  let of_bytes bytes =
    ...
    | model :: dim_str :: usage_str :: emb_strs ->
        ...
        let embeddings = List.filter_map (fun s ->
    ...
    | _ ->
        { model = "unknown"; dimension = 0; usage = 0; embeddings = [] }
end
...
let print_embedding_info idx emb =
  ...
  let preview_str = Array.to_list preview
...
let () =
  ...
  let result = Grpc_eio.Pool.with_connection ~sw ~env pool (fun client ->
  ...
  | Ok response_bytes ->
      ...
      if List.length response.embeddings >= 4 then begin
        let emb1 = List.nth response.embeddings 0 in  (* OCaml functional *)
        let emb4 = List.nth response.embeddings 3 in  (* Functional programming *)
        let emb2 = List.nth response.embeddings 1 in  (* gRPC *)

        Printf.printf "\n🔍 Similarity analysis:\n";
        Printf.printf "   'OCaml functional' vs 'Functional programming': %.4f\n"
          (cosine_similarity emb1 emb4);
        Printf.printf "   'OCaml functional' vs 'gRPC microservices': %.4f\n"
  ...
  Printf.printf "\n📊 Pool stats: acquired=%d, released=%d, created=%d\n"
    stats.acquired_count stats.released_count stats.created_count;
