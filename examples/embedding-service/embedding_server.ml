...
module EmbedResponse = struct
  ...
  let to_bytes t =
    ...
    let dim = match t.embeddings with
      | [] -> 0
      | e :: _ -> Array.length e
    in
    let emb_strs = List.map (fun e ->
      Array.to_list e
...
module EmbeddingModel = struct
  ...
  let embed_batch texts =
    List.map embed_text texts
...
let embed (request_bytes : string) : string =
  ...
  Eio.traceln "📊 [Embed] Processing %d texts with model %s"
    (List.length request.texts) request.model;
  ...
  let usage = List.fold_left (fun acc t -> acc + EmbeddingModel.count_tokens t) 0 request.texts in

  Eio.traceln "📊 [Embed] Generated %d embeddings in %.3fs (%d tokens)"
    (List.length embeddings) elapsed usage;
...
let () =
  ...
  Grpc_eio.Health.set_status health ~service:"" Grpc_eio.Health.Serving;  (* overall *)
