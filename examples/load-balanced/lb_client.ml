...
let parse_response response =
  ...
  match parts with
  | msg :: rest ->
      let from_server = List.find_map (fun p ->
        if String.length p > 5 && String.sub p 0 5 = "from:" then
...
let () =
  ...
  let balancer = Grpc_eio.Balancer.create
  ...
  for i = 1 to num_requests do
    ...
    try
      let response = Grpc_eio.Balancer.call ~sw ~env balancer (fun client ->
