...
module ChatResponse = struct
  ...
  let of_bytes bytes =
    if String.length bytes > 7 && String.sub bytes (String.length bytes - 7) 7 = "|FINISH" then
      (* Has finish marker *)
      let parts = String.split_on_char '|' bytes in
      match parts with
      | token :: rest ->
          let finish = List.find_opt (fun s -> String.length s > 7 && String.sub s 0 7 = "FINISH:") rest in
          { token;
            finish_reason = Option.map (fun s -> String.sub s 7 (String.length s - 7)) finish;
            is_done = Option.is_some finish }
      | _ -> { token = bytes; finish_reason = None; is_done = false }
    else
      { token = bytes; finish_reason = None; is_done = false }
end
...
let run_chat env prompt =
  ...
  (* Build request *)
  let request = ChatRequest.create
    ~model:"simulated-gpt"
    ~max_tokens:50
    ~temperature:0.7
    prompt
