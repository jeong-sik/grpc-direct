...
module ChatRequest = struct
  ...
  let of_bytes bytes =
    ...
    | _ ->
        { model = "default"; prompt = bytes; max_tokens = 100; temperature = 0.7 }
end
...
module AIModel = struct
  let generate_response ~prompt ~max_tokens ~temperature:_ =
    ...
    let responses = [
      ...
      "here"; "'s"; " my"; " response"; ":"; " ";
      "This"; " is"; " a"; " streaming"; " AI"; " response"; ".";
    ] in
    let tokens = if max_tokens < List.length responses
...
let chat_completion (request_bytes : string) : string Grpc_eio.Stream.t =
  ...
  let prompt_preview = String.sub request.prompt 0 (min 30 (String.length request.prompt)) in
  Eio.traceln "🤖 [AI] Generating response for: %s..." prompt_preview;

  let tokens = AIModel.generate_response
  ...
  List.iteri (fun i token ->
    ...
    let response = ChatResponse.{
      token;
      finish_reason = if is_last then Some "stop" else None;
    } in
...
let metrics_interceptor () : string Grpc_eio.Interceptor.t =
  ...
  Grpc_eio.Interceptor.make ~name:"metrics" (fun ctx next ->
    ...
    Eio.traceln "📊 [Metrics] Request #%d completed in %.3fs" !request_count elapsed;
    result
  )
...
let () =
  ...
  Printf.printf "  dune exec examples/ai-chat/chat_client.exe -- \"Your question here\"\n%!";
