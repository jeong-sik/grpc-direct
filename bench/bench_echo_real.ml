...
let echo_handler _addr (reqd : H2.Reqd.t) =
  ...
  then (
        ...
        ~on_eof:(fun () ->
          ...
          let response =
            H2.Response.create
              ~headers:
                (H2.Headers.of_list
                   [ "content-type", "application/grpc+proto"; "grpc-status", "0" ])
              `OK
          in
    ...
    (* Unknown method *)
    let response =
      H2.Response.create ~headers:(H2.Headers.of_list [ "grpc-status", "12" ]) `OK
    in
...
let run_benchmark env =
  ...
  printf "═══════════════════════════════════════════════════════\n";
  printf
    "  grpc-direct: %.0f RPS, P50=%.0f µs, P99=%.0f µs\n"
    rps
    (ns_to_us (percentile stats 0.50))
    (ns_to_us (percentile stats 0.99));
  printf "═══════════════════════════════════════════════════════\n\n%!";
...
let () = Eio_main.run run_benchmark
