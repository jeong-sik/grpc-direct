...
module Proto = struct
  ...
  type streaming_input_request = { payload : payload option }
  ...
  let encode_payload (p : payload) =
    ...
    key_type
    ^ encode_varint (payload_type_to_int p.payload_type)
    ^ key_body
    ^ body_len
    ^ p.body
  ;;
  ...
  let encode_streaming_output_response payload = encode_simple_response payload
...
let () =
  ...
  let usage =
    "interop_official_server --host 127.0.0.1 --port 50051 [--cert cert.pem --key \
     key.pem --ca ca.pem] [--gzip]"
  in
  Arg.parse
  ...
  let streaming_output_call bytes =
    ...
    let stream =
      Grpc_eio.Stream.create (max 1 (List.length req.Proto.response_parameters))
    in
    List.iter
      (fun p ->
    ...
    Eio.Fiber.fork ~sw (fun () ->
      let rec loop () =
        try
          ...
          loop ()
        with
        | Eio.Time.Timeout -> Grpc_eio.Stream.close response_stream
