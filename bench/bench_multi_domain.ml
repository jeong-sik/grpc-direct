...
let make_echo_service () =
  ...
  Grpc_eio.Service.create
    ~name:"echo.Echo"
    ~methods:[
      Grpc_eio.Service.unary "Echo" handler
    ]
    ()
...
let run_single_domain_server env =
  ...
  let server = Grpc_eio.Server.create ~config ()
...
let run_multi_domain_server ~domains env =
  ...
  Eio.Fiber.fork_daemon ~sw:(Eio.Switch.run (fun sw -> sw)) (fun () ->
    while not !stop do
...
let () =
  ...
  Printf.printf "   2. Terminal 2: ghz --insecure --proto bench/go-comparison/echo.proto \\\n";
  Printf.printf "                  --call echo.Echo/Echo -d '{\"message\":\"hello\"}' \\\n";
  Printf.printf "                  -n 10000 -c 50 127.0.0.1:50051\n";
  Printf.printf "\n   For multi-domain:\n";
  Printf.printf "   1. Terminal 1: dune exec bench/bench_server.exe -- --multi --domains 4\n";
