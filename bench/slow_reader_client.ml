  ...
  try
    ...
    let rec drain count =
      ...
      | Error status ->
        ...
        then printf "Stream completed after %d messages\n%!" count
        else printf "Stream error (%s) after %d messages\n%!" status.message count
      | exception End_of_file -> printf "Stream ended after %d messages\n%!" count
    in
    let rec loop count =
      ...
      then (
        ...
        match Grpc_eio.Stream.take stream with
        | Ok bytes ->
          if !print_every > 0 && count mod !print_every = 0
          then (
            let msg =
        ...
        | exception End_of_file -> printf "Stream ended after %d messages\n%!" count)
    in
    Fun.protect ~finally:(fun () -> Grpc_eio.Client.close client) (fun () -> loop 0);
