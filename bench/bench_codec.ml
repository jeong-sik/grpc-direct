...
let time_it name iterations f =
  ...
  Printf.printf
    "%-30s %8d ops  %8.2f µs/op  %8.0f ops/sec\n%!"
    name
    iterations
    per_op
...
let random_string len = String.init len (fun _ -> Char.chr (Random.int 256))
...
let () =
  ...
  let encoded_small =
    match Grpc_core.Message.encode ~codec small_msg with
    | Ok s -> s
    | Error e -> failwith e
  in
  let encoded_medium =
    match Grpc_core.Message.encode ~codec medium_msg with
    | Ok s -> s
    | Error e -> failwith e
  in
  let encoded_gzip =
    match Grpc_core.Message.encode ~codec:gzip medium_msg with
    | Ok s -> s
    | Error e -> failwith e
  in
  ...
  (* Status creation *)
  time_it "Status.ok" 500_000 (fun () -> Grpc_core.Status.ok);
  time_it "Status.error" 500_000 (fun () ->
    Grpc_core.Status.error Grpc_core.Status.Internal "error");
  (* Timeout parsing *)
  time_it "Timeout.parse" 500_000 (fun () -> Grpc_core.Timeout.parse "30S");
