...
module GrpcFrame = struct
  ...
  let decode data =
    ...
    then None
      ...
      let len =
        (Char.code data.[1] lsl 24)
        lor (Char.code data.[2] lsl 16)
        lor (Char.code data.[3] lsl 8)
        lor Char.code data.[4]
      in
      if String.length data < 5 + len then None else Some (String.sub data 5 len))
  ;;
...
let echo_handler _addr (reqd : H2.Reqd.t) =
  ...
  if String.equal path "/echo.EchoService/Echo"
  then (
      ...
      H2.Body.Reader.schedule_read
        ...
        ~on_eof:(fun () ->
          ...
          let response_payload =
            match GrpcFrame.decode request_data with
...
let run_single_domain env =
  ...
  while true do
    Eio.Net.accept_fork
      ~sw
      socket
      ~on_error:(fun exn -> printf "Connection error: %s\n%!" (Printexc.to_string exn))
...
let run_multi_domain ~domains env =
  ...
  let domain_worker id () =
    ...
    let addr = `Tcp (Eio.Net.Ipaddr.V4.loopback, port) in
    let socket =
      Eio.Net.listen ~sw ~backlog:128 ~reuse_addr:true ~reuse_port:true net addr
    in
  ...
  while true do
    Eio.Net.accept_fork
      ~sw
      socket
      ~on_error:(fun exn -> printf "Domain 0 error: %s\n%!" (Printexc.to_string exn))
      (fun client_socket client_addr ->
         Eio.Switch.run
         @@ fun inner_sw ->
         H2_eio.Server.create_connection_handler
...
let () =
  ...
  Arg.parse
    ...
    (fun _ -> ())
    "grpc-direct benchmark server";
  Eio_main.run
  @@ fun env ->
  if !multi then run_multi_domain ~domains:!domains env else run_single_domain env
;;
