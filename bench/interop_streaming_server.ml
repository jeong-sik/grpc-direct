...
let () =
  ...
  let usage =
    "interop_streaming_server --host 127.0.0.1 --port 50051 \
     [--cert cert.pem --key key.pem --ca ca.pem] [--gzip] [--metrics] [--metrics-port 9464] \
     [--metrics-any] [--stream-count 100] [--stream-delay 0.0] [--shutdown-after 0]"
  in
  Arg.parse
    [
      ...
      ("--metrics-port", Arg.Set_int metrics_port, "Prometheus metrics port (default: 9464)");
      ...
      ("--stream-delay", Arg.Set_float stream_delay, "Delay per server-streaming message (seconds)");
      ("--shutdown-after", Arg.Set_float shutdown_after, "Shutdown after N seconds (0 = no)");
    ]
    (fun _ -> ())
    usage;
  ...
  Eio.Switch.run @@ fun sw ->
    ...
    let unary_handler bytes =
      let message = match decode_payload bytes with
        | Some msg -> msg
        | None -> ""
      in
    ...
    (match tls with
     ...
     | Some _ -> Printf.printf "Interop streaming server (TLS) listening on %s:%d\n%!" !host !port);

    if !shutdown_after > 0.0 then
      Eio.Fiber.fork ~sw (fun () ->
        let clock = Eio.Stdenv.clock env in
    ...
    Option.iter (fun metrics_registry ->
      let addr =
        if !metrics_any then
          `Tcp (Eio.Net.Ipaddr.V4.any, !metrics_port)
        else
          `Tcp (Eio.Net.Ipaddr.V4.loopback, !metrics_port)
      in
      Eio.Fiber.fork ~sw (fun () ->
        Grpc_eio.Metrics.serve_prometheus ~addr ~sw ~env metrics_registry
      );
      Printf.printf "Prometheus metrics on %s:%d/metrics\n%!"
