  ...
  then false
    ...
    let rec loop i =
      ...
      then true
        ...
        let c = name.[i] in
        if c >= 'A' && c <= 'Z' then false else loop (i + 1))
    in
    loop 0)
;;
...
let validate_request_headers ~is_trailers headers =
  ...
  let add_pseudo name value =
    if Hashtbl.mem seen_pseudo name
    then raise (Header_error ("duplicate pseudo-header " ^ name));
  ...
  let rec loop = function
    | [] -> ()
    | (name, value) :: rest ->
      if not (is_lowercase_header_name name)
      then raise (Header_error ("invalid header name " ^ name));
      if is_pseudo name
      then (
        if is_trailers then raise (Header_error "pseudo-header in trailers");
        if !seen_regular then raise (Header_error "pseudo-header after regular headers");
        ...
        | "keep-alive"
        | "transfer-encoding"
        | "upgrade" -> raise (Header_error ("connection-specific header " ^ name))
        | "te" ->
          if String.lowercase_ascii (String.trim value) <> "trailers"
          then raise (Header_error "invalid te header")
        | "content-length" -> add_content_length value
        | _ -> ());
      loop rest
  in
  loop headers;
  ...
  then (
    ...
    | Some method_value ->
      if method_value = "" then raise (Header_error "empty :method");
      if String.uppercase_ascii method_value = "CONNECT"
      then (
        if !authority = None then raise (Header_error "missing :authority for CONNECT");
        if !scheme <> None || !path <> None
        then raise (Header_error "invalid CONNECT pseudo-headers"))
      else (
        if !scheme = None then raise (Header_error "missing :scheme");
        if !path = None then raise (Header_error "missing :path");
        match !path with
  ...
  | [] -> None
  | first :: rest ->
    if List.for_all (fun v -> v = first) rest
    then Some first
    else raise (Header_error "conflicting content-length")
;;
...
let () =
  ...
  while true do
    Eio.Net.accept_fork
      ~sw
      socket
      ~on_error:(fun exn -> eprintf "Connection error: %s\n%!" (Printexc.to_string exn))
      (fun client_socket _client_addr ->
         ...
         let finalize_stream state =
           if Stream.is_closed state.stream.stream_state
           then remove_stream state.stream.id
         in
         let try_send_pending_data state =
           ...
           | Some data ->
             ...
             then (
               ...
               let chunk =
                 if to_send = data_len then data else Cstruct.sub data 0 to_send
               in
               ...
               then (
                 ...
                 state.pending_data
                 <- Some (Cstruct.sub data to_send (data_len - to_send)))
         in
         let handle_data frame =
           let stream_id = frame.Frame.header.stream_id in
           if stream_id = 0l
           then connection_error Frame.Error_code.protocol_error "DATA on stream 0";
           ...
           | Some state ->
             ...
             in
             (try Stream.transition_recv state.stream.stream_state `Data ~end_stream with
              | Stream.Stream_error _ ->
                connection_error Frame.Error_code.stream_closed "DATA on closed stream");
             ...
             let data_len =
               ...
               then (
                 if payload_len < 1
                 then
                   connection_error Frame.Error_code.protocol_error "Invalid DATA padding";
                 let pad_len = Cstruct.get_uint8 payload 0 in
                 if pad_len > payload_len - 1
                 then
                   connection_error Frame.Error_code.protocol_error "Invalid DATA padding";
                 payload_len - 1 - pad_len)
               else payload_len
             in
             let updates = Flow_control.consume_recv flow_control stream_id data_len in
             if updates <> []
             then (
               let update_frames =
                 List.map
         ...
         let handle_headers frame =
           ...
           if stream_id = 0l
           then connection_error Frame.Error_code.protocol_error "HEADERS on stream 0";
           let existing = Hashtbl.find_opt streams stream_id in
           let is_new = existing = None in
           if is_new
           then (
             if Int32.rem stream_id 2l = 0l
             then
               connection_error
                 Frame.Error_code.protocol_error
                 "client-initiated stream id must be odd";
             if Int32.compare stream_id !max_stream_id_seen <= 0
             then
               connection_error Frame.Error_code.stream_closed "HEADERS on closed stream";
             max_stream_id_seen := stream_id);
           let can_accept =
             if
               is_new
               && Hashtbl.length streams
                  >= conn.Connection.local_settings.max_concurrent_streams
             then (
               send_rst_stream conn flow_control stream_id Frame.Error_code.refused_stream;
               false)
             else true
           in
           if not can_accept
         ...
         let handle_settings frame =
           ...
           if Frame.Flags.is_set frame.Frame.header.flags Frame.Flags.ack
           then (
             if payload_len <> 0
             then
               connection_error
                 Frame.Error_code.frame_size_error
                 "SETTINGS ack with payload")
           else (
             if payload_len mod 6 <> 0
             then
               connection_error
                 Frame.Error_code.frame_size_error
             ...
             List.iter
               (fun (id, value) ->
                  if
                    id = Frame.Settings_id.initial_window_size
                    && Int32.logand value 0x80000000l <> 0l
                  then
                    connection_error
             ...
             | Failure msg -> connection_error Frame.Error_code.protocol_error msg
         ...
         let handle_window_update frame =
           ...
           then
             ...
             then
               ...
               send_rst_stream conn flow_control stream_id Frame.Error_code.protocol_error;
               remove_stream stream_id)
           else (
             let stream_known = Hashtbl.mem streams stream_id in
             if stream_id <> 0l && not stream_known
             then
               if Int32.compare stream_id !max_stream_id_seen <= 0
               then ()
               else
               ...
               | Failure msg when starts_with msg "FLOW_CONTROL_ERROR" ->
                 if stream_id = 0l
                 then connection_error Frame.Error_code.flow_control_error msg
                 else (
                   send_rst_stream
                     conn
                     flow_control
                   ...
                   remove_stream stream_id));
             if stream_id = 0l
             then Hashtbl.iter (fun _ state -> try_send_pending_data state) streams
             else (
               match Hashtbl.find_opt streams stream_id with
         ...
         let handle_priority frame =
           let stream_id = frame.Frame.header.stream_id in
           if stream_id = 0l
           then connection_error Frame.Error_code.protocol_error "PRIORITY on stream 0";
           let prio, _ = Frame.parse_priority frame.Frame.payload ~offset:0 in
           if prio.dependency = stream_id || prio.weight < 1 || prio.weight > 256
           then (
             send_rst_stream conn flow_control stream_id Frame.Error_code.protocol_error;
             if Hashtbl.mem streams stream_id then remove_stream stream_id)
           else (
             (match Hashtbl.find_opt streams stream_id with
         ...
         try
           ...
           let rec loop () =
             ...
             | Frame.RstStream ->
               ...
               if stream_id = 0l
               then
                 connection_error Frame.Error_code.protocol_error "RST_STREAM on stream 0";
               if Cstruct.length frame.Frame.payload <> 4
               then
                 connection_error
                   Frame.Error_code.frame_size_error
               ...
               loop ()
             | Frame.Continuation ->
               connection_error Frame.Error_code.protocol_error "Unexpected CONTINUATION"
             | Frame.PushPromise ->
               connection_error Frame.Error_code.protocol_error "PUSH_PROMISE not allowed"
             | _ -> loop ()
           in
           loop ()
         with
         ...
         | Connection.Connection_error (code, msg) -> send_goaway conn code msg
         | Invalid_argument msg -> send_goaway conn Frame.Error_code.protocol_error msg
         | Failure msg -> send_goaway conn Frame.Error_code.protocol_error msg)
  done
;;
