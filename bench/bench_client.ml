...
let record_latency stats latency_ns =
  if stats.count < Array.length stats.latencies
  then (
...
let generate_payload size = String.init size (fun i -> Char.chr (65 + (i mod 26)))
  ...
  for _ = 1 to 100 do
    let _, elapsed =
      time_ns (fun () ->
        ...
        @@ fun sw ->
        let client =
          Grpc_eio.Client.connect
            ~sw
            ~env
            (sprintf "http://%s:%d" config.host config.port)
        in
...
let bench_unary_latency ~sw ~env ~config =
  ...
  let client =
    Grpc_eio.Client.connect ~sw ~env (sprintf "http://%s:%d" config.host config.port)
  in
...
let print_comparison rps p50_us p99_us =
  ...
  printf "├────────────────────┼───────────┼───────────┼───────────┤\n";
  printf
    "│ OCaml grpc-direct │ %9.0f │ %9.2f │ %9.2f │\n"
    rps
    (ns_to_us p50_us)
    (ns_to_us p99_us);
  printf "├────────────────────┼───────────┼───────────┼───────────┤\n";
...
let bench_dry_run config =
  ...
  for _ = 1 to 100000 do
    let _, elapsed =
      time_ns (fun () -> ignore (Grpc_eio.Client.Credentials.get_call_headers creds))
    in
...
let parse_args () =
  ...
  let specs =
    ...
    ; ( "--requests"
      , Arg.Int (fun i -> config := { !config with requests = i })
      , "Number of requests" )
    ; ( "--concurrency"
      , Arg.Int (fun i -> config := { !config with concurrency = i })
      , "Concurrency level" )
    ; ( "--warmup"
      , Arg.Int (fun i -> config := { !config with warmup = i })
      , "Warmup requests" )
    ; ( "--payload"
      , Arg.Int (fun i -> config := { !config with payload_size = i })
