...
|---------------|---------|---------|----------|----------|-------|
| grpc-go | 41.5k | 2.10 ms | 55.9k | 5.91 ms | Go 1.21 |
| tonic (Rust) | 39.0k | 2.42 ms | 54.1k | 6.03 ms | Rust 1.92 + tokio |
...
| grpc-eio | 21.9k | 3.80 ms | 22.0k | 14.10 ms | OCaml 5 + Eio |
...
- Raw logs: `tmp/ghz_*_r{1..3}_<ts>.txt` (timestamp in `tmp/bench_ts.txt`).
...
- **Date**: 2026-01-17 (Latest cross-language measurements)
- **Aggregation**: median of 3 runs per implementation (see `tmp/bench_ts.txt`)
...
|----------------|--------------|-------------|-------------|---------|-------|
| grpc-go | **41,486** | **0.68 ms** | **2.10 ms** | 106.4% | Go 1.21 |
| tonic (Rust) | 38,998 | 0.82 ms | 2.42 ms | 100% | Rust 1.92 + tokio |
| grpc/grpc (C++) | 37,947 | 0.92 ms | 2.17 ms | 97.3% | grpc-cpp 1.76 |
| grpc-js (Node) | 24,598 | 1.37 ms | 6.22 ms | 63.1% | Node 18 + grpc-js |
| grpc-dotnet | 30,962 | 1.19 ms | 2.47 ms | 79.4% | .NET 8 (docker) |
| grpc-eio | 21,851 | 1.68 ms | 3.80 ms | 56.0% | OCaml 5 + Eio |
| grpc-java | 28,250 | 1.34 ms | 3.32 ms | 72.4% | Java 21 + grpc-java |
| grpcio (Python) | 9,394 | 5.04 ms | 11.64 ms | 24.1% | Python 3.13 + grpcio |

#### High Concurrency Test (200 concurrent, 50,000 requests)

| Implementation | Measured RPS | P50 Latency | P99 Latency | vs Rust | Notes |
|----------------|--------------|-------------|-------------|---------|-------|
| grpc-go | **55,911** | **2.71 ms** | **5.91 ms** | 103.4% | Go 1.21 |
| tonic (Rust) | 54,059 | 2.91 ms | 6.03 ms | 100% | Rust 1.92 + tokio |
| grpc/grpc (C++) | 41,840 | 3.98 ms | 7.06 ms | 77.4% | grpc-cpp 1.76 |
| grpc-js (Node) | 39,603 | 4.35 ms | 9.74 ms | 73.3% | Node 18 + grpc-js |
| grpc-dotnet | 46,615 | 3.53 ms | 11.94 ms | 86.2% | .NET 8 (docker) |
| grpc-eio | 22,036 | 7.69 ms | 14.10 ms | 40.8% | OCaml 5 + Eio |
| grpc-java | 43,046 | 4.31 ms | 6.40 ms | 79.6% | Java 21 + grpc-java |
| grpcio (Python) | 9,987 | 19.55 ms | 29.04 ms | 18.5% | Python 3.13 + grpcio |

#### Performance Ratios Summary

| OCaml vs | Standard Test | High Concurrency |
|----------|---------------|------------------|
| **Rust tonic** | 56.0% | 40.8% |
| **Go grpc-go** | 52.7% | 39.4% |

**Key Findings (2026-01-17 Update, median of 3 runs):**
1. **Go/Rust lead; C++/Java/.NET/Node form the next cluster** and swap order by load.
2. **grpc-eio is ~56% of Rust (std) / ~41% (high)** and ~53%/~39% of Go.
